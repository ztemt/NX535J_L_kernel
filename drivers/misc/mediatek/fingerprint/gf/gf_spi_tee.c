...
#define GF_NETLINK_ROUTE 29   /* for GF test only, need defined in include/uapi/linux/netlink.h */
...
#define CUST_EINT_FP_EINT_NUM			   10
...
#define GF_INPUT_FF_KEY  KEY_F11
...
#endif
	if(g_gf_dev->is_power_on == bonoff) {
		gf_debug(INFO_LOG, "%s return directly\n", __func__);
		return;
	}
...
static void gf_spi_clk_enable(u8 bonoff)
{
	if(g_gf_dev == NULL)
	{
		printk("g_gf_dev is null.\n");
		return;
	}
    static int count = 0;
	if (bonoff && (count == 0))
	{
		nubia_enable_spi_clk(g_gf_dev->spi,true);
		count = 1;
	} else if ((count > 0) && (bonoff == 0)) {
		nubia_enable_spi_clk(g_gf_dev->spi,false);
		count = 0;
	}
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case GF_IOC_EXIT:
		if (gf_dev->system_status == 0) {
			gf_debug(INFO_LOG, "%s: system already-exit ======\n", __func__);
			gf_dev->device_available = 0;
			break;
		}
		gf_disable_irq(gf_dev);
		if (gf_dev->spi->irq && gf_dev->irq_available == true) {
			free_irq(gf_dev->spi->irq, gf_dev);
			gf_dev->irq_count = 0;
			gf_dev->irq_available = false;
		}
		gf_dev->device_available = 0;
	...
	case GF_IOC_ENABLE_POWER:
		gf_debug(INFO_LOG, "%s: enable power.\n",__func__);
		gf_hw_power_enable(1);
		break;

	case GF_IOC_DISABLE_POWER:
		gf_debug(INFO_LOG, "%s: disable power.\n",__func__);
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		if(GF_KEY_POWER == gf_key.key) {
			gf_debug(INFO_LOG, "%s: received KEY_F11\n", __func__);
			//input_report_key(gf_dev->input, KEY_F11, 1);
			//input_sync(gf_dev->input);
			//input_report_key(gf_dev->input, KEY_F11, 0);
			//input_sync(gf_dev->input);
		} else {
			gf_debug(INFO_LOG, "%s: received KEY_F3, key=%d, value=%d\n", __func__, gf_key.key, gf_key.value);
			input_report_key(gf_dev->input, KEY_F3, gf_key.value);
			input_sync(gf_dev->input);
		}
