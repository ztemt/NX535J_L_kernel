	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < ESD_CHECK_NUM; i++) {
			...
			DISPDBG("[DSI]enter cmp read_data0 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x\n",
			     ...
			     read_data0.byte3);
			DISPERR("[DSI]enter cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x,para_list[2]=0x%x\n",
			...
			packet_type = read_data0.byte0;
			DISPERR("DSI read packet_type is 0x%x \n",packet_type);
			DISPERR("[DSI]enter cmp read_data0.byte1=0x%x read_data1.byte0=0x%x  read_data1.byte1=0x%x read_data1.byte2=0x%x \n",read_data0.byte1,read_data1.byte0,read_data1.byte1,read_data1.byte2);
                       if(packet_type == 0x1A || packet_type == 0x1C) {
				recv_data_cnt = read_data0.byte1 + read_data0.byte2 * 16;
				if(recv_data_cnt > 10) {
				...
				DISPCHECK("DSI read long packet size: %d\n", recv_data_cnt);
                       if ((read_data0.byte1 == 0x01)&&(read_data1.byte0 == dsi_params->lcm_esd_check_table[i].para_list[0])) {
                                ret = 0;        /* esd pass */
                        } else if ((read_data0.byte1 == 0x03)&&(read_data1.byte0 == dsi_params->lcm_esd_check_table[i].para_list[0])&&(read_data1.byte1 == dsi_params->lcm_esd_check_table[i].para_list[1])&&(read_data1.byte2 == dsi_params->lcm_esd_check_table[i].para_list[2])) {
				/* clear rx data */
