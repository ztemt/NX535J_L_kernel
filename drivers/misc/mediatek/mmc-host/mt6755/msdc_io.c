	...
	void __iomem **base, unsigned int *irq, struct msdc_hw **hw)
{
	...
	//printk(KERN_ERR"alexander:into msdc_dt_init! cd_irq is %d\n",*cd_irq);
...
skip_cust_node:
	...
	if ((pdev->id == 1) &&
		...
		if (eint_node) {
			//printk(KERN_ERR"alexander:find MSDC1_INS-eint node!!\n");
			(*hw)->flags |= (MSDC_CD_PIN_EN | MSDC_REMOVABLE);//when impl hotplug,uncomment it,when not hotplug,please comment it.xiaofeng add

			/* get irq #  */
			if (!(*cd_irq)) {
				*cd_irq = irq_of_parse_and_map(eint_node, 0);
				//printk(KERN_ERR"alexander:cd_irq from dtsi is %d\n",*cd_irq);
				//printk(KERN_ERR"alexander:from MTKlog can't irq_of_parse_and_map for card detect eint!!\n");//mtk NOSENSE LOG
			} else {
				//printk(KERN_ERR"alexander:into cdirq else branch\n");
