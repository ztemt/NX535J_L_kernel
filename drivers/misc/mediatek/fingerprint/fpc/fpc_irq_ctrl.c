...
int fpc_irq_ctrl_init(fpc_irq_data_t *fpc_irq_data, fpc_irq_pdata_t *pdata)
{
	...
	fpc_irq_data->pdata.rst_gpio = pdata->rst_gpio;
	{
		...
		return error;
	      ...
	      error = mt_set_gpio_pull_select(pdata->rst_gpio, GPIO_PULL_UP);
            if (error != 0) {
		    printk("[FPC]GPIO_PULL_UP(reset) failed.\n");
		    return error;
		} else {
			printk("[FPC]GPIO_PULL_UP(reset) success.\n");
		}
        
	      error = mt_set_gpio_pull_enable(pdata->rst_gpio, GPIO_PULL_ENABLE);
            if (error != 0) {
		    printk("[FPC]GPIO_PULL_ENABLE(reset) failed.\n");
		    return error;
		} else {
			printk("[FPC]GPIO_PULL_ENABLE(reset) success.\n");
		}
		...
		mt_set_gpio_out(fpc_irq_data->pdata.rst_gpio, 1);
             ...
             fpc_irq_ctrl_hw_reset(fpc_irq_data);
	}
	return error;
...
static void fpc_irq_ctrl_reset_out(fpc_irq_data_t *fpc_irq_data)
{
	...
	printk("fpc reset is finish==========lfw.");
}
