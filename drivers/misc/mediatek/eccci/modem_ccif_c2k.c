...
static void ccif_rx_work(struct work_struct *work)
{
	...
	struct md_ccif_queue *queue =
      ...
      //Nubia add for debug, according to MTK advice
	CCCI_REP_MSG(queue->modem->index, TAG, "Q%u rx collect start\n", queue->index);
      //Nubia END
	ret = ccif_rx_collect(queue, queue->budget, 1, &result);
      //Nubia add for debug, according to MTK advice
	CCCI_REP_MSG(queue->modem->index, TAG, "Q%u rx collect end,ret %d\n", queue->index,ret);
      //Nubia END
