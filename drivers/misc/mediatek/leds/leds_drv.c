		...
		struct device_attribute *attr, const char *buf,size_t count)
{
	...
	case 0:
		...
		pmic_set_register_value(PMIC_ISINK_DIM0_DUTY, 12);
	...
	case 1:
		...
		pmic_set_register_value(PMIC_ISINK_DIM1_DUTY, 12);
	...
	case 2:
		...
		pmic_set_register_value(PMIC_ISINK_DIM4_DUTY, 12);
	...
	case 3:
		...
		pmic_set_register_value(PMIC_ISINK_DIM5_DUTY, 12);
...
static ssize_t outn_11_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	return count;
...
static ssize_t outn_11_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(outn_11, 0664, outn_11_show, outn_11_store);
...
static int __init mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		if((strcmp(g_leds_data[i]->cust.name, "LED_RIGHT") !=0) && (strcmp(g_leds_data[i]->cust.name, "LED_LEFT") !=0) && (strcmp(g_leds_data[i]->cust.name, "INDICATOR-R") !=0) && (strcmp(g_leds_data[i]->cust.name, "INDICATOR-G") !=0))
		{
...
static int mt65xx_leds_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_group(&pdev->dev.kobj, &aw2013_attr_group);
