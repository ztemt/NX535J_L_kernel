...
{
	int i4RetValue = 0;
    //resonant_frequency = 175; 1 ~ 255   0.4*175+30hz=100Hz
    u16 resonant_frequency = 175;  // 100hz need modify every module
    u16 ISRC_value = 1;
    ...
    //ZTEMT: added by congshan start
	char puSendCmd_step_time[2] = {(char)(((step_time >> 8) & 0x03) | 0xDC), (char)(step_time & 0xff)};

    i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd_step_time, 2);
    if (i4RetValue < 0) {
		LOG_INF("I2C send failed!!\n");
		return -1;
	}
	char puSendCmd_step_width[2] = {(char)(((step_width >> 8) & 0x03) | 0xE4), (char)(step_width & 0xff)};

    i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd_step_width, 2);
    if (i4RetValue < 0) {
		LOG_INF("I2C send failed!!\n");
		return -1;
	}
