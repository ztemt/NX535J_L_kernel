...
{
	int ret; kal_int32 value;
    union power_supply_propval val;
    struct power_supply	*usb_psy = power_supply_get_by_name("usb");

    if(usb_psy)
    {
        ret = usb_psy->get_property(usb_psy, POWER_SUPPLY_PROP_CHARGE_DETECTION, &val);
        value = val.intval;
    }

	if(value == USB_SDP_CHARGER)
		return STANDARD_HOST;
	if(value == USB_DCP_CHARGER || value == USB_HVDCP_CHARGER || value == USB_CDP_CHARGER)
		return CHARGING_HOST;

    return value;
};

static inline void BATTERY_SetUSBState(int usb_state) { return;}
...
static inline bool upmu_is_chr_det(void)
{
    ...
    if(usb_psy)
    {
        ret = usb_psy->get_property(usb_psy, POWER_SUPPLY_PROP_ONLINE, &val);
        value = val.intval;
    }
