...
#if NUBIA_TEEI_DEBUG
#define NUBIA_TEEI_DBG(fmt, args...) printk("\033[;34m[TEEI][TZDriver][switch_fn]"fmt"\033[0m", ##args)
#else
#define NUBIA_TEEI_DBG(fmt,args...)
#endif
#define NUBIA_TEEI_MSG(fmt, args...) printk("\033[;34m[TEEI][TZDriver][switch_fn]"fmt"\033[0m", ##args)
...
static struct switch_call_struct *create_switch_call_struct(void)
{
	...
	if (tmp_entry == NULL)
		NUBIA_TEEI_MSG("[%s][%d] kmalloc failed!!!\n", __func__, __LINE__);
...
{
	if ((ent == NULL)) {
		NUBIA_TEEI_MSG("[%s][%d] the paraments are wrong!\n", __func__, __LINE__);
		return -EINVAL;
	}
...
int add_work_entry(int work_type, unsigned long buff)
{
	...
	if (retVal != 0) {
		NUBIA_TEEI_MSG("[%s][%d] with wrong work_type!\n", __func__, __LINE__);
		return retVal;
	}

	work_entry = create_switch_call_struct();
	if (work_entry == NULL) {
		NUBIA_TEEI_MSG("[%s][%d] There is no enough memory!\n", __func__, __LINE__);
		return -ENOMEM;
	}

	retVal = init_switch_call_struct(work_entry, work_type, buff);
	if (retVal != 0) {
		NUBIA_TEEI_MSG("[%s][%d] init_switch_call_struct failed!\n", __func__, __LINE__);
...
int del_work_entry(struct switch_call_struct *ent)
{
	...
	if (retVal != 0) {
		NUBIA_TEEI_MSG("[%s][%d] del_switch_call_link failed %d!\n", __func__, __LINE__, retVal);
		return retVal;
	}

	retVal = destroy_switch_call_struct(ent);
	if (retVal != 0) {
		NUBIA_TEEI_MSG("[%s][%d] destroy_switch_call_struct failed %d!\n", __func__, __LINE__, retVal);
		return retVal;
	}
	return 0;
}
...
static void switch_fn(struct kthread_work *work)
{
	...
	case CAPI_CALL:
		...
		if (retVal < 0) {
			NUBIA_TEEI_MSG("[%s][%d] fail to handle ClientAPI!\n", __func__, __LINE__);
		}
	...
	case FDRV_CALL:
		...
		if (retVal < 0) {
			NUBIA_TEEI_MSG("[%s][%d] fail to handle F-driver!\n", __func__, __LINE__);
		}
		break;
	case BDRV_CALL:
		retVal = handle_bdrv_call(switch_ent->buff_addr);
		if (retVal < 0) {
			NUBIA_TEEI_MSG("[%s][%d] fail to handle B-driver!\n", __func__, __LINE__);
		}
		break;
	case SCHED_CALL:
		retVal = handle_sched_call(switch_ent->buff_addr);
		if (retVal < 0) {
			NUBIA_TEEI_MSG("[%s][%d] fail to handle sched-Call!\n", __func__, __LINE__);
		}
		break;
	default:
		NUBIA_TEEI_MSG("switch fn handles a undefined call! [%d] \n", call_type);
	}
