...
static ssize_t fts_ic_detect_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    disable_irq(client->irq);
    rc =fts_test_entry("FT5436_V02_20160130.ini");
    if(rc < 0){
...
static DEVICE_ATTR(easy_wakeup_gesture, S_IWUSR|S_IXUSR|S_IWGRP|S_IXGRP|S_IWOTH|S_IXOTH, NULL, fts_easy_wakeup_gesture_store);
