...
#ifdef NORMAL_GPIO
#define GPIO_I_TREE_PIN         (GPIO83 | 0x80000000)
#define GPIO_I_TREE_RX         (GPIO42 | 0x80000000)
#define GPIO_I_TREE_TX         (GPIO43 | 0x80000000)
#endif
...
/*
**  change tx rx to gpio, and pull down
*/
static void i_three_pull_down_txrx(void){
	mt_set_gpio_mode(GPIO_I_TREE_TX, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO_I_TREE_TX, GPIO_DIR_OUT);
	mt_set_gpio_pull_enable(GPIO_I_TREE_TX, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_I_TREE_TX, GPIO_PULL_DOWN);
	mt_set_gpio_out(GPIO_I_TREE_TX, 0);

	mt_set_gpio_mode(GPIO_I_TREE_RX, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO_I_TREE_TX, GPIO_DIR_OUT);
	mt_set_gpio_pull_enable(GPIO_I_TREE_RX, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_I_TREE_RX, GPIO_PULL_DOWN);
	mt_set_gpio_out(GPIO_I_TREE_RX, 0);
}
/*
**  set tx rx to uart, and pull up
*/
static void i_three_pull_up_txrx(void){
	mt_set_gpio_mode(GPIO_I_TREE_TX, GPIO_MODE_02);
	mt_set_gpio_dir(GPIO_I_TREE_TX, GPIO_DIR_OUT);
	mt_set_gpio_pull_enable(GPIO_I_TREE_TX, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_I_TREE_TX, GPIO_PULL_UP);

	mt_set_gpio_mode(GPIO_I_TREE_RX, GPIO_MODE_02);
	mt_set_gpio_dir(GPIO_I_TREE_TX, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_I_TREE_RX, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_I_TREE_RX, GPIO_PULL_UP);
}
static int i_three_power_init(struct i_three_chip *chip)
{
	int rc;
#ifdef NORMAL_GPIO
	chip->power_gpio = GPIO_I_TREE_PIN;
	mt_set_gpio_mode(chip->power_gpio, GPIO_MODE_00);
	mt_set_gpio_dir(chip->power_gpio, GPIO_DIR_OUT);
	mt_set_gpio_out(chip->power_gpio, 0);

	i_three_pull_down_txrx();
#else
...
static int i_three_power_on(struct i_three_chip *chip, bool enable)
{
	...
	if (enable) {
#ifdef NORMAL_GPIO
		i_three_pull_up_txrx();

		SENSOR_LOG_INFO("i_three_power_on:GPIO_PULL_UP\n");
		mt_set_gpio_out(chip->power_gpio, 1);
		rc = 0;
#else
		...
		SENSOR_LOG_INFO("i_three_power_on:GPIO_PULL_DOWN\n");
		mt_set_gpio_out(chip->power_gpio, 0);
		rc = 0;
#else
		...
		struct device_attribute *attr,	const char *buf, size_t size)
{
	...
	SENSOR_LOG_ERROR("attr_enable_ir_store start\n");
