...
PowerUp PowerOnList = {
	{
	 {SENSOR_DRVNAME_IMX258_MIPI_RAW_MAIN_541,
	  {
	 ...
	 {SENSOR_DRVNAME_IMX258_MIPI_RAW_SUB_541,
	  {
	   {SensorMCLK, Vol_High, 0},
	   {DOVDD, Vol_1800, 0},
	   {AVDD, Vol_2800, 0},
...
BOOL hwpowerdown(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		...
		//if (pinSetIdx == 0){
			...
			if (mt_set_gpio_out
			    (pPowerCustList->PowerCustInfo[0].Gpio_Pin,
			     //ZTEMT:wangkai add for power down GPIO for AVDD when power down sensor---Start
			     //pPowerCustList->PowerCustInfo[0].Voltage)) {
			     Vol_Low)) {
			     //ZTEMT:wangkai add for power down GPIO for AVDD when power down sensor---End
				PK_DBG("[CAMERA LENS] set gpio failed!!\n");
			}
		}
	} else if (pwInfo.PowerType == DVDD) {
