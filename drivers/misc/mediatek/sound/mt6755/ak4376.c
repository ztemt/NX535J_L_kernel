...
#define GPIO16_HIFI_PDN   (GPIO22 | 0x80000000)
...
//	ak4376_data = ak4376;
	...
	if (i2c->dev.platform_data)	{
		...
		printk("\t[AK4376] %s(%d) pdata set is valid\n",__FUNCTION__,__LINE__);

		if (gpio_is_valid(ak4376->priv_pdn_en)) {
			printk("\t[AK4376] %s(%d) pdn_en is valid\n",__FUNCTION__,__LINE__);
			ret2 = gpio_request(ak4376->priv_pdn_en, "pdn_en");
			if (ret2) {
				printk("\t[AK4376] %s(%d) cannot get pdn_en gpio\n",__FUNCTION__,__LINE__);
				ak4376->pdn1 = 2;	//No use GPIO control
			} else {
				ret2 = gpio_direction_output(ak4376->priv_pdn_en, 0);
				if (ret2) {
					printk("\t[AK4376] %s(%d) pdn_en=0 fail\n", __FUNCTION__,__LINE__);
					gpio_free(ak4376->priv_pdn_en);
					ak4376->pdn1 = 2;
				} else {
					dev_dbg(&i2c->dev, "vad_clock_en=0\n");
					printk("\t[AK4376] %s(%d) pdn_en=0\n", __FUNCTION__,__LINE__);
				}
			}
		} else {
			printk("\t[AK4376] %s(%d) pdn_en is invalid\n", __FUNCTION__,__LINE__);
			ak4376->pdn1 = 2;
		}
	}
#endif	
	akdbgprt("\t[AK4376] %s(%d) probe ok\n", __FUNCTION__,__LINE__);
...
#define GPIO_HIFI_LDO_EN_PIN (GPIO21| 0x80000000)
