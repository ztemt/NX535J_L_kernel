...
#define CAMERA_AVDD_EN_PIN        GPIO_CAMERA_LDO_EN_PIN 

PowerCust PowerCustList = {
	{
//	 {GPIO24, GPIO_MODE_GPIO, Vol_High},	/* for AVDD; */
...
PowerCust PowerCustList1 = {
	{
	 {CAMERA_AVDD_EN_PIN, GPIO_MODE_GPIO, Vol_High},	/* for AVDD; */
//	 {GPIO_UNSUPPORTED, GPIO_MODE_GPIO, Vol_Low},	/* for AVDD; */
	 {GPIO_UNSUPPORTED, GPIO_MODE_GPIO, Vol_Low},	/* for DVDD; */
	 {GPIO_UNSUPPORTED, GPIO_MODE_GPIO, Vol_Low},	/* for DOVDD; */
	 {GPIO_UNSUPPORTED, GPIO_MODE_GPIO, Vol_Low},	/* for AFVDD; */
...
PowerCust *pPowerCustList = NULL;

PowerUp PowerOnList = {
	{
	 {SENSOR_DRVNAME_IMX258_MIPI_RAW_MAIN_537,
	  {
	 ...
	 {SENSOR_DRVNAME_IMX179_MIPI_RAW_SUB_537,
	  {
	   {SensorMCLK, Vol_High, 0},
	   {DOVDD, Vol_1800, 0},
	   {AVDD, Vol_2800, 0},
...
BOOL hwpoweron(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		if (pPowerCustList->PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (TRUE != _hwPowerOn(pwInfo.PowerType, pwInfo.Voltage)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
				return FALSE;
			}
		} else {
			if (mtkcam_gpio_set(pinSetIdx, CUST_AVDD, pPowerCustList->PowerCustInfo[CUST_AVDD].Voltage)) {
					PK_DBG("[CAMERA CUST_AVDD] set gpio failed!!\n");
			}
		}
	} else if (pwInfo.PowerType == DVDD) {
		if (pinSetIdx == 2) {
			if (pPowerCustList->PowerCustInfo[CUST_MAIN2_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (mtkcam_gpio_set(pinSetIdx, CUST_MAIN2_DVDD, pPowerCustList->PowerCustInfo[CUST_MAIN2_DVDD].Voltage)) {
						PK_DBG("[CAMERA CUST_MAIN2_DVDD] set gpio failed!!\n");
				}
			}
		} else if (pinSetIdx == 1) {
			if (pPowerCustList->PowerCustInfo[CUST_SUB_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				/* PK_DBG("[CAMERA SENSOR] Sub camera VCAM_D power on"); */
				if (pwInfo.Voltage == Vol_1200) {
					pwInfo.Voltage = Vol_1220;
				...
				if (mtkcam_gpio_set(pinSetIdx, CUST_SUB_DVDD, pPowerCustList->PowerCustInfo[CUST_SUB_DVDD].Voltage)) {
					PK_DBG("[CAMERA CUST_SUB_DVDD] set gpio failed!!\n");
				}
			}
		} else {
			if (pPowerCustList->PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (mtkcam_gpio_set(pinSetIdx, CUST_DVDD, pPowerCustList->PowerCustInfo[CUST_DVDD].Voltage)) {
					PK_DBG("[CAMERA CUST_DVDD] set gpio failed!!\n");
				}
			}
		}
	} else if (pwInfo.PowerType == DOVDD) {
		if (pPowerCustList->PowerCustInfo[CUST_DOVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (TRUE != _hwPowerOn(pwInfo.PowerType, pwInfo.Voltage)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
				return FALSE;
			}
		} else {
			if (mtkcam_gpio_set(pinSetIdx, CUST_DOVDD, pPowerCustList->PowerCustInfo[CUST_DOVDD].Voltage)) {
		...
		if (pPowerCustList->PowerCustInfo[3].Gpio_Pin == GPIO_UNSUPPORTED) {
			...
			if (mt_set_gpio_mode
			    (pPowerCustList->PowerCustInfo[3].Gpio_Pin,
			     pPowerCustList->PowerCustInfo[3].Gpio_Mode)) {
				PK_DBG("[CAMERA SENSOR] set gpio mode failed!!\n");
			}
			if (mt_set_gpio_dir(pPowerCustList->PowerCustInfo[3].Gpio_Pin, GPIO_DIR_OUT)) {
				PK_DBG("[CAMERA SENSOR] set gpio dir failed!!\n");
			}
			if (mt_set_gpio_out
			    (pPowerCustList->PowerCustInfo[3].Gpio_Pin,
			     pPowerCustList->PowerCustInfo[3].Voltage)) {
				PK_DBG("[CAMERA SENSOR] set gpio failed!!\n");
			}

			if (pPowerCustList->PowerCustInfo[4].Gpio_Pin != GPIO_UNSUPPORTED) {
				mdelay(5);
				if (mt_set_gpio_mode
				    (pPowerCustList->PowerCustInfo[3].Gpio_Pin,
				     pPowerCustList->PowerCustInfo[3].Gpio_Mode)) {
				...
				if (mt_set_gpio_dir
				    (pPowerCustList->PowerCustInfo[3].Gpio_Pin, GPIO_DIR_OUT)) {
					PK_DBG("[CAMERA SENSOR] set gpio dir failed!!\n");
				}
				if (mt_set_gpio_out
				    (pPowerCustList->PowerCustInfo[3].Gpio_Pin,
...
BOOL hwpowerdown(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		if (pPowerCustList->PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			...
			if (mtkcam_gpio_set(pinSetIdx, CUST_AVDD, 1-pPowerCustList->PowerCustInfo[CUST_DOVDD].Voltage)) {
					PK_DBG("[CAMERA CUST_AVDD] set gpio failed!!\n");/* 1-voltage for reverse*/
			}
		}
	} else if (pwInfo.PowerType == DVDD) {
		if (pPowerCustList->PowerCustInfo[1].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (pinSetIdx == 1) {
...
#if 0
			if (mt_set_gpio_mode
			    (pPowerCustList->PowerCustInfo[1].Gpio_Pin,
			     pPowerCustList->PowerCustInfo[1].Gpio_Mode)) {
				PK_DBG("[CAMERA LENS] set gpio mode failed!!\n");
			}
			if (mt_set_gpio_dir(pPowerCustList->PowerCustInfo[1].Gpio_Pin, GPIO_DIR_OUT)) {
				PK_DBG("[CAMERA LENS] set gpio dir failed!!\n");
			}
			if (mt_set_gpio_out
			    (pPowerCustList->PowerCustInfo[1].Gpio_Pin,
			     pPowerCustList->PowerCustInfo[1].Voltage)) {
				PK_DBG("[CAMERA LENS] set gpio failed!!\n");
			}
#endif
		}
	} else if (pwInfo.PowerType == DOVDD) {
		if (pPowerCustList->PowerCustInfo[2].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
				return FALSE;
			}
		} else {
			if (mtkcam_gpio_set(pinSetIdx, CUST_DOVDD, 1-pPowerCustList->PowerCustInfo[CUST_DOVDD].Voltage)) {
				PK_DBG("[CAMERA CUST_AVDD] set gpio failed!!\n");/* 1-voltage for reverse*/
			}
		}
	} else if (pwInfo.PowerType == AFVDD) {
...
{   	
	if (pwInfo.PowerType == AVDD) {
		...
		if (pPowerCustList->PowerCustInfo[0].Gpio_Pin == GPIO_UNSUPPORTED) {
	//	if(pinSetIdx == 0){
			if (TRUE != hwPowerOn(pwInfo.PowerType, pwInfo.Voltage * 1000, mode_name)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
				return FALSE;
			...
			if (mt_set_gpio_mode
			    (pPowerCustList->PowerCustInfo[0].Gpio_Pin,
			     pPowerCustList->PowerCustInfo[0].Gpio_Mode)) {
				PK_DBG("[CAMERA SENSOR] set gpio mode failed!!\n");
			}
			if (mt_set_gpio_dir(pPowerCustList->PowerCustInfo[0].Gpio_Pin, GPIO_DIR_OUT)) {
				PK_DBG("[CAMERA SENSOR] set gpio dir failed!!\n");
			}
			if (mt_set_gpio_out
			    (pPowerCustList->PowerCustInfo[0].Gpio_Pin,
			     pPowerCustList->PowerCustInfo[0].Voltage)) {
				PK_DBG("[CAMERA SENSOR] set gpio failed!!\n");
			}
		}
	} else if (pwInfo.PowerType == DVDD) {
		...
		if (pPowerCustList->PowerCustInfo[2].Gpio_Pin == GPIO_UNSUPPORTED) {
			...
			if (mt_set_gpio_mode
			    (pPowerCustList->PowerCustInfo[2].Gpio_Pin,
			     pPowerCustList->PowerCustInfo[2].Gpio_Mode)) {
				PK_DBG("[CAMERA SENSOR] set gpio mode failed!!\n");
			}
			if (mt_set_gpio_dir(pPowerCustList->PowerCustInfo[2].Gpio_Pin, GPIO_DIR_OUT)) {
				PK_DBG("[CAMERA SENSOR] set gpio dir failed!!\n");
			}
			if (mt_set_gpio_out
			    (pPowerCustList->PowerCustInfo[2].Gpio_Pin,
			     pPowerCustList->PowerCustInfo[2].Voltage)) {
				PK_DBG("[CAMERA SENSOR] set gpio failed!!\n");
			}
		}
	} else if (pwInfo.PowerType == AFVDD) {
...
BOOL hwpowerdown(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		...
		PK_DBG("[CAMERA SENSOR] Skip AFVDD setting\n");
		//ZTEMT:wangkai do not power down AFVDD/LDO28 for audio use the same LDO28----Start
		#if 0
		if (pPowerCustList->PowerCustInfo[3].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (TRUE != hwPowerDown(pwInfo.PowerType, mode_name)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
		...
		#endif
		//ZTEMT:wangkai do not power down AFVDD/LDO28 for audio use the same LDO28----End
	} else if (pwInfo.PowerType == PDN) {
		PK_DBG("hwPowerDown: PDN %d\n", pwInfo.Voltage);
		       ...
		       char *mode_name)
{
	...
	if(pinSetIdx == 1)
              pPowerCustList = &PowerCustList1;
	else
              pPowerCustList = &PowerCustList;
