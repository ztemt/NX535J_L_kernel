...
LCM_DRIVER *cabc_lcm_drv = NULL;
struct kobject *lcm_cabc_kobj;
int lcm_cabc_sysfs_init(void);
void lcm_cabc_sysfs_exit(void);
...
int disp_lcm_init(disp_lcm_handle *plcm, int force)
{
	...
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
        if(lcm_drv->set_cabc && lcm_drv->get_cabc){
            cabc_lcm_drv = plcm->drv;
            lcm_cabc_sysfs_init();
            }
...
{
    unsigned int cabc_level = CABC_LEVER1;
        ...
        struct kobj_attribute *attr, const char *buf, size_t size)
{
    ...
    if ((val != CABC_OFF) && (val != CABC_LEVER1) && (val != CABC_LEVER2) && (val != CABC_LEVER3)) {
        printk("%s: invalid cabc val = %d\n", __func__, val);
        return size;
	}
        if (cabc_lcm_drv->is_lcmon)cabc_lcm_drv->is_lcmon(&lcm_isok);
...
int lcm_cabc_sysfs_init(void)
{
	...
	printk("lcm_cabc_create_sysfs success!\n");
...
void lcm_cabc_sysfs_exit(void)
{
	...
	sysfs_remove_group(lcm_cabc_kobj,&lcm_attribute_group);

	kobject_del(lcm_cabc_kobj);
}
