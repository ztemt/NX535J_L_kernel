...
int fts_i2c_read_testa(/*struct i2c_client *client,*/ char *writebuf,int writelen, char *readbuf, int readlen)
{
    return fts_i2c_read(fts_i2c_client, writebuf, writelen, readbuf, readlen);
}

int fts_i2c_write_testa(/*struct i2c_client *client,*/ char *writebuf, int writelen)
{
...
static int tpd_touchinfo(struct ts_event *data)
{
    ...
    if(fts_palm_buf!=buf[1]){
        fts_palm_buf=buf[1];
        data->palm_flag = buf[1];
        fts_palm_flag=0;
        printk( "palm_flag = %x \n",data->palm_flag);
        }
...
#else // MT_PROTOCOL_B
    if(data->palm_flag == 0x55)
    {
    if(fts_palm_flag==0){
        fts_palm_flag=1;
        printk( " report palm_flag = %x \n",data->palm_flag);
        input_report_key(tpd->dev, BTN_TOUCH, 1);
...
reset_proc:
    ...
    */
    ...
    request_irq(TPD_EINT_TOUCH_PANEL_NUM, tpd_eint_interrupt_handler, IRQF_TRIGGER_NONE, "TOUCH_PANEL-eint", NULL);
    disable_irq(TPD_EINT_TOUCH_PANEL_NUM);
...
static void fts_release_all_finger()
{
