...
#define imx298_I2C_SPEED 400

static DEFINE_SPINLOCK(imgsensor_drv_lock);

kal_bool Is_longExp=0;  //tanyijun add 
...
static imgsensor_info_struct imgsensor_info = {
    ...
    .i2c_addr_table = {0x34, 0x34, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};
   ...
   if(Is_longExp)
    { 
        ...
        // m_LineTimeInus[0] = (MUINT32)(((MUINT64)pFeaturePara16[0]*1000000 + ((FeaturePara32/1000)-1))/(FeaturePara32/1000)); 
         ...
         shutter_time = shutter_time * 9227;
	    // 四舍五入 取整
		 ...
		 // set clock plck = ( (24M*84)/(5*2*4) ) * 4(lane) = 201 600 000
		        ...
		        // set LINE_LENGTH_PCK = 5536
		         write_cmos_sensor(0x0342, 0x15);
                 write_cmos_sensor(0x0343, 0xA0);
                 // set  FRM_LENGH_LINES = 31700
		         write_cmos_sensor(0x0340, 0x7B);
                 ...
                 write_cmos_sensor(0x0823, 0x00);
                   ///201 600 000 /5535/31700=1.14
		          /// 1s 5536*31700*1.14/5536 = 36416	  
		    line_fix = 36138;
		  ...
		  // set FRM_LENGH_LINES = 36100
		   write_cmos_sensor(0x0340, 0x8D);
                 write_cmos_sensor(0x0341, 0x04);				 
                   ///201 600 000 /5535/36100=1
		    /// 1s 5535/36100/5536 = 36100	
                line_fix = 36100;
            }
            else if(shutter_timeS >= 129 && shutter_timeS < 257)
            {
                // set clock plck = ( (24M*84)/(5*2*4) ) * 4(lane) = 201 600 000
                 ...
                 write_cmos_sensor(0x0822, 0x00);
                 write_cmos_sensor(0x0823, 0x00);
		  // set LINE_LENGTH_PCK = 32752
		   write_cmos_sensor(0x0342, 0x7F);
                 write_cmos_sensor(0x0343, 0xF0);
                 //201 600 000/32752/12100 = 0.5  
		  // set  FRM_LENGH_LINES = 12100
		   write_cmos_sensor(0x0340, 0x2F);
		  ...
		  // set LINE_LENGTH_PCK = 32752
		   write_cmos_sensor(0x0342, 0x7F);
                 write_cmos_sensor(0x0343, 0xF0);
                 //201 600 000/32752/23700 = 0.25  
		  // set  FRM_LENGH_LINES = 23700
		   write_cmos_sensor(0x0340, 0x5C);
		  ...
		  // set LINE_LENGTH_PCK = 32752
		   write_cmos_sensor(0x0342, 0x7F);
                 write_cmos_sensor(0x0343, 0xF0);
                 //201 600 000/32752/48900 = 0.125  
		  // set  FRM_LENGH_LINES = 48900
		   write_cmos_sensor(0x0340, 0xBF);
                 write_cmos_sensor(0x0341, 0x04);				 
		  ...
		  // set LINE_LENGTH_PCK = 32752
		         write_cmos_sensor(0x0342, 0x7F);
                 write_cmos_sensor(0x0343, 0xF0);
                 //201 600 000/32752/48900 = 0.1  
		  // set  FRM_LENGH_LINES = 56000
		         write_cmos_sensor(0x0340, 0xDA);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    spin_unlock(&imgsensor_drv_lock);
    LOG_INF("tanAE gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor(0x0112,0x0A);
	write_cmos_sensor(0x0113,0x0A);
	write_cmos_sensor(0x034C,0x09); //2328
	write_cmos_sensor(0x034D,0x18);
	write_cmos_sensor(0x034E,0x06);//1748
	write_cmos_sensor(0x034F,0xD4);
	write_cmos_sensor(0x0401,0x00);
	write_cmos_sensor(0x0404,0x00);
	write_cmos_sensor(0x0405,0x10);
