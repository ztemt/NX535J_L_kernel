...
int cal_time(struct timeval start_time, struct timeval end_time)
{
	...
	NUBIA_TEEI_MSG("function expend %d u-seconds! \n", timestamp);
	return timestamp;
}
...
static int printer_thread_function(unsigned long virt_addr, unsigned long para_vaddr, unsigned long buff_vaddr)
{
	...
	if (retVal < 0) {
		...
		if (retVal == 0)
			NUBIA_TEEI_MSG("[SEM status] Printer App is not RUNNING\n");
		else if (retVal == 1)
			NUBIA_TEEI_MSG("[SEM status] BLUE Printer is not Ready\n");
...
static long register_shared_param_buf(struct service_handler *handler)
{
	...
	if (message_buff == NULL) {
		NUBIA_TEEI_MSG("[%s][%d]: There is NO command buffer!.\n", __func__, __LINE__);
		return -EINVAL;
	}


	if (handler->size > VDRV_MAX_SIZE) {
		NUBIA_TEEI_MSG("[%s][%d]: The vDrv buffer is too large, DO NOT Allow to create it.\n", __FILE__, __LINE__);
		return -EINVAL;
	}
	...
	if (handler->param_buf == NULL) {
		NUBIA_TEEI_MSG("[%s][%d]: kmalloc vdrv_buffer failed.\n", __FILE__, __LINE__);
		return -ENOMEM;
	}
...
static void load_func(struct work_struct *entry)
{
	...
	get_online_cpus();
	cpu_id = get_current_cpuid();
	NUBIA_TEEI_DBG("[%s][%d]current cpu id[%d] \n", __func__, __LINE__, cpu_id);
...
static int init_all_service_handlers(void)
{
	...
	/* socket.init(&socket); */
	NUBIA_TEEI_DBG("[%s][%d] begin to init reetime service!\n", __func__, __LINE__);
	reetime.init(&reetime);
	NUBIA_TEEI_DBG("[%s][%d] init reetime service successfully!\n", __func__, __LINE__);

	NUBIA_TEEI_DBG("[%s][%d] begin to init vfs service!\n", __func__, __LINE__);
	vfs_handler.init(&vfs_handler);
	NUBIA_TEEI_DBG("[%s][%d] init vfs service successfully!\n", __func__, __LINE__);
...
static long create_notify_queue(unsigned long msg_buff, unsigned long size)
{
	...
	if (size > MAX_BUFF_SIZE) {
		NUBIA_TEEI_MSG("[%s][%d]: The NQ buffer size is too large, DO NOT Allow to create it.\n", __FILE__, __LINE__);
		retVal = -EINVAL;
		goto return_fn;
	}
	...
	if (nt_t_buffer == NULL) {
		NUBIA_TEEI_MSG("[%s][%d]: kmalloc nt_t_buffer failed.\n", __func__, __LINE__);
		retVal =  -ENOMEM;
		goto return_fn;
	}
	...
	if (t_nt_buffer == NULL) {
		NUBIA_TEEI_MSG("[%s][%d]: kmalloc t_nt_buffer failed.\n", __func__, __LINE__);
		retVal =  -ENOMEM;
...
static long create_ctl_buffer(unsigned long msg_buff, unsigned long size)
{
	...
	/* Check the argument */
	if (size > MAX_BUFF_SIZE) {
		NUBIA_TEEI_MSG("[%s][%d]: The CTL buffer size is too large, DO NOT Allow to create it.\n", __FILE__, __LINE__);
		return -EINVAL;
	}
	...
	if (sys_ctl_buffer == NULL) {
		NUBIA_TEEI_MSG("[%s][%d]: kmalloc ctl_buffer failed.\n", __FILE__, __LINE__);
		return -ENOMEM;
	}
...
static int create_nq_buffer(void)
{
	...
	if (retVal < 0) {
		NUBIA_TEEI_MSG("[%s][%d]:create_notify_queue failed with errno %d.\n", __func__, __LINE__, retVal);
		return -EINVAL;
	}
...
void set_fp_command(unsigned long memory_size)
{
	...
	NUBIA_TEEI_DBG("[%s][%d]", __func__, __LINE__);
...
static irqreturn_t nt_switch_irq_handler(void)
{
	...
	if (boot_soter_flag == START_STATUS) {
		...
		if (FAST_CALL_TYPE == msg_head->message_type) {
			...
			NUBIA_TEEI_MSG("[%s][%d] ==== Unknown IRQ ========\n", __func__, __LINE__);
			return IRQ_NONE;
		}
	}
}
...
int register_switch_irq_handler(void)
{
	...
	if (retVal)
		NUBIA_TEEI_MSG("[CONFIG_OF] [%s] ERROR for request_irq %d error code : %d.\n", __func__, irq_num, retVal);
	else
	...
	if (retVal)
		NUBIA_TEEI_MSG("ERROR for request_irq %d error code : %d.\n", SWITCH_IRQ, retVal);
	else
...
static int create_vDrv_vfs_buffer(void)
{
	...
	if (retVal < 0)
		NUBIA_TEEI_MSG("[%s][%d]:create_vdrv_buffer failed with errno %d.\n", __func__, __LINE__, retVal);
...
static void secondary_init_cmdbuf(void *info)
{
	...
	rmb();
		...
		NUBIA_TEEI_DBG("[%s][%d] message = %lx,  fdrv message = %lx, bdrv_message = %lx, tlog_message = %lx.\n", __func__, __LINE__,
...
long create_cmd_buff(void)
{
	...
	if (message_buff == NULL) {
		NUBIA_TEEI_MSG("[%s][%d] Create message buffer failed!\n", __FILE__, __LINE__);
		return -ENOMEM;
	}
	...
	if (fdrv_message_buff == NULL) {
		NUBIA_TEEI_MSG("[%s][%d] Create fdrv message buffer failed!\n", __FILE__, __LINE__);
	...
	if (bdrv_message_buff == NULL) {
		NUBIA_TEEI_MSG("[%s][%d] Create bdrv message buffer failed!\n", __FILE__, __LINE__);
		free_pages(message_buff, get_order(ROUND_UP(MESSAGE_LENGTH, SZ_4K)));
	...
	if (tlog_message_buff == NULL) {
		NUBIA_TEEI_MSG("[%s][%d] Create tlog message buffer failed!\n", __FILE__, __LINE__);
		free_pages(message_buff, get_order(ROUND_UP(MESSAGE_LENGTH, SZ_4K)));
		free_pages(fdrv_message_buff, get_order(ROUND_UP(MESSAGE_LENGTH, SZ_4K)));
...
unsigned long create_fp_fdrv(int buff_size)
{
	...
	if (buff_size > VDRV_MAX_SIZE) {
		NUBIA_TEEI_MSG("[%s][%d]: FP Drv buffer is too large, Can NOT create it.\n", __FILE__, __LINE__);
		return NULL;
	}
	...
	if (temp_addr == NULL) {
		NUBIA_TEEI_MSG("[%s][%d]: kmalloc fp drv buffer failed.\n", __FILE__, __LINE__);
		return NULL;
	}
...
int teei_service_init(void)
{
	...
	NUBIA_TEEI_DBG("[%s][%d] begin to create nq buffer!\n", __func__, __LINE__);
	create_nq_buffer();
	NUBIA_TEEI_DBG("[%s][%d] end of creating nq buffer!\n", __func__, __LINE__);

	if (soter_error_flag == 1)
		return -1;

	NUBIA_TEEI_DBG("[%s][%d] begin to create fp buffer!\n", __func__, __LINE__);
