...
{
    struct mxt_data *data = dev_get_drvdata(dev);
    dev_info(dev, "set touch_mode case %s \n",buf);
    int inv = -1,ret = -1;
    ...
    printk(dev,"touch mode set done\n");
    return count;
...
static DEVICE_ATTR(update_cfg, S_IWUSR|S_IXUSR|S_IWGRP|S_IXGRP|S_IWOTH|S_IXOTH , NULL, mxt_update_cfg_store);
...
static DEVICE_ATTR(easy_wakeup_gesture, S_IWUSR|S_IXUSR|S_IWGRP|S_IXGRP|S_IWOTH|S_IXOTH, NULL, mxt_easy_wakeup_gesture_store);
...
#	endif
static DEVICE_ATTR(en_glove, S_IRUSR|S_IXUSR|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH, mxt_plugin_glove_show, NULL);
