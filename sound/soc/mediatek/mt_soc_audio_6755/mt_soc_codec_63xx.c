...
{
      return;//added by nubia, for gpio24 can not use in hp
...
{
	//pr_err("+mt63xx_codec_prepare_forak4376\n");
	
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		//pr_err("mt63xx_codec_prepare_forak4376 set up  SNDRV_PCM_STREAM_CAPTURE rate = %d\n", substream->runtime->rate);
	       mBlockSampleRate[AUDIO_ANALOG_DEVICE_IN_ADC] = substream->runtime->rate;
	} else if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		//pr_err("mt63xx_codec_prepare_forak4376 set up  SNDRV_PCM_STREAM_PLAYBACK rate = %d\n", substream->runtime->rate);
               mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] = substream->runtime->rate;
