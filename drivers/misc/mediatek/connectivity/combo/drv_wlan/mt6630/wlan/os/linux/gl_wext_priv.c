...
#if CFG_NUBIA_WEXT_PRIV_CMD_STRUCT
/*
* MTK init define of priv_driver_cmd_t is different with android_wifi_priv_cmd
* in mediatek_driver_cmd_nl80211.c,
* this will cause priv_support_driver_cmd get wrong value of priv_driver_cmd_t->total_len
* and then priv_support_driver_cmd failed.
*
* So nubia modify priv_driver_cmd_t define as android_wifi_priv_cmd in mediatek_driver_cmd_nl80211.c
*
...
int priv_support_driver_cmd(IN struct net_device *prNetDev, IN OUT struct ifreq *prReq, IN int i4Cmd)
{
	...
	if (!prReq->ifr_data) {
		ret = -EINVAL;
#if CFG_NUBIA_WEXT_PRIV_CMD_STRUCT
		DBGLOG(REQ, WARN, "%s: ifr_data null\n", __func__);
#endif
		goto exit;
	}
...
#if CFG_NUBIA_WEXT_PRIV_CMD_STRUCT
		DBGLOG(REQ, WARN, "%s: u4ReadyFlag 0\n", __func__);
#endif
		ret = -EINVAL;
		goto exit;
	}
	...
	if (i4TotalLen <= 0) {
#if CFG_NUBIA_WEXT_PRIV_CMD_STRUCT
		DBGLOG(REQ, WARN, "%s: i4TotalLen <= 0\n", __func__);
#endif
		ret = -EINVAL;
		goto exit;
	}
...
#if CFG_NUBIA_WEXT_PRIV_CMD_STRUCT
	pcCommand = kmemdup(priv_cmd.buf, i4TotalLen, GFP_KERNEL);
	if (!pcCommand) {
		DBGLOG(REQ, INFO, "%s: failed to kmemdup memory size %d\n", __func__, i4TotalLen);
		ret = -ENOMEM;
		goto exit;
	}
#else
...
#if CFG_NUBIA_WEXT_PRIV_CMD_STRUCT
		memcpy(priv_cmd.buf, pcCommand, i4BytesWritten);
#else
