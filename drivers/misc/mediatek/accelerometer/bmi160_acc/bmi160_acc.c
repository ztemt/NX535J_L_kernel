...
/*
bool __attribute__((weak)) hwPowerOn(MT65XX_POWER powerId, MT65XX_POWER_NONE powerVolt, char *mode_name)
{
	return 0;
}
*/
...
int bmi160_m_get_data(int* x ,int* y,int* z, int* status)
{
	...
	*status = sensor_data[7];
	...
	printk("%s x: %d, y: %d, z: %d,", __func__, sensor_data[4], sensor_data[5], sensor_data[6]);
	mutex_unlock(&sensor_data_mutex);
...
int bmi160_o_get_data(int* x ,int* y,int* z, int* status)
{
	...
	*status = sensor_data[11];
	...
	printk("%s x: %d, y: %d, z: %d,", __func__, sensor_data[8], sensor_data[9], sensor_data[10]);
	mutex_unlock(&sensor_data_mutex);
