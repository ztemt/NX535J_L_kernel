...
#define OV8856_I2C_SPEED 400
#define OV8856_MAX_OFFSET		0xFFFF
...
static bool selective_read_eeprom(kal_uint16 addr, BYTE* data)
{
    ...
    if(addr > OV8856_MAX_OFFSET)
        return false;
	kdSetI2CSpeed(OV8856_I2C_SPEED);

	if(iReadRegI2C(pu_send_cmd, 2, (u8*)data, 1, OV8856_EEPROM_READ_ID)<0)
		return false;
    return true;
}
...
static int selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size)
{
	...
	while (size_to_read > 0) {
		...
		if (byteread_cmos_sensor(0x6d, curAddr, buff)) {
...
static int iWriteData(unsigned int  ui4_offset, unsigned int  ui4_length,unsigned char pinputdata)
{
  ...
  //    pinputdata = &para;
	...
	// 	                                                   ui4_offset,ui4_length,pinputdata);
       ...
       kdSetI2CSpeed(OV8856_I2C_SPEED); // Add this func to set i2c speed by each sensor
	char pu_send_cmd[3] = {(char)(ui4_offset >> 8), (char)(ui4_offset & 0xFF), (char)(pinputdata & 0xFF)};
	iWriteRegI2C(pu_send_cmd, 3, 0x6c);
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		i4RetValue = selective_read_region(ptempbuf->u4Offset, pWorkingBuff,OV8856_EEPROM_READ_ID, ptempbuf->u4Length);
#if 0
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclient->addr =OV8856_EEPROM_READ_ID >> 1;
...
MODULE_DESCRIPTION("ov8856 CAM_CAL driver");
