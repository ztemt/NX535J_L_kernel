...
extern void mt_wifi_newant_power_on();
extern void vcn33_ldo_power_off();
...
int do_wlan_drv_init(int chip_id)
{
	...
	case 0x6630:
		...
		if((0==strcmp(CONFIG_ARCH_MTK_PROJECT,"NX535J")))
		{
			WMT_DETECT_ERR_FUNC("nubia:%s,disable vcn33 according to power colleage\n",__func__);
			vcn33_ldo_power_off();
		}
		WMT_DETECT_ERR_FUNC("alexander:power new ant in %s\n",__func__);
		//mt_wifi_newant_power_on();
#else
