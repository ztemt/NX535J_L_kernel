...
static void pa22x_load_calibration_param(struct i2c_client *client)
{
	...
	if(pa22x_read_file(PS_CAL_FILE_PATH,buftemp) < 0)
	{
		...
		APS_ERR("Use Default near count = %d\n", obj->near_diff_cnt);
	}
	else
	{
...
static int pa22x_run_calibration(struct i2c_client *client)
{
	...
	if (data->crosstalk == 0)
	{
		APS_ERR("%s: proximity sensor calibration error!\n", __func__);
		return -1;
	}
	...
	{
		APS_ERR("%s: invalid calibrated data\n", __func__);
	...
	{
		APS_ERR("Open PS calibration file error!!");
		return -1;
	}
	else
	{
...
int pa22x_enable_ps(struct i2c_client *client, int enable)
{
	...
	if(enable == 1)
	{
		...
		if(obj->hw->polling_mode_ps == 0) //0:interrupt mode
		{
			...
			//PS On
			hwmsen_write_byte(client,REG_CFG0, regdata | 0x02); 
			mutex_unlock(&obj->update_lock);
			//Refresh data
			msleep(50);
		...
		mutex_lock(&obj->update_lock);
		hwmsen_write_byte(client,REG_CFG0, regdata);
...
/*---------------------------------------------------------------------------------------*/
static DRIVER_ATTR(version,     S_IWUGO | S_IRUGO, pa22x_show_version, NULL);

static DRIVER_ATTR(ps,      S_IWUGO | S_IRUGO, pa22x_show_ps, NULL);
static DRIVER_ATTR(config,  S_IWUGO | S_IRUGO, pa22x_show_config,	pa22x_store_config);
//#ifndef DEVICE_PA224
static DRIVER_ATTR(alslv,   S_IWUGO | S_IRUGO, pa22x_show_alslv, pa22x_store_alslv);
static DRIVER_ATTR(alsval,  S_IWUGO | S_IRUGO, pa22x_show_alsval, pa22x_store_alsval);
static DRIVER_ATTR(als,     S_IWUGO | S_IRUGO, pa22x_show_als, NULL);
//#endif
static DRIVER_ATTR(status,  S_IWUGO | S_IRUGO, pa22x_show_status, NULL);
static DRIVER_ATTR(send,    S_IWUGO | S_IRUGO, pa22x_show_send, pa22x_store_send); // No func
static DRIVER_ATTR(recv,    S_IWUGO | S_IRUGO, pa22x_show_recv, pa22x_store_recv);    // No func
static DRIVER_ATTR(reg,     S_IWUGO | S_IRUGO, pa22x_show_reg, NULL);
static DRIVER_ATTR(psoffset, S_IWUGO | S_IRUGO, pa22x_show_ps_offset,pa22x_set_ps_offset);
static DRIVER_ATTR(pscalibration, S_IWUGO | S_IRUGO, pa22x_show_ps_calibration,pa22x_set_ps_calibration);
static DRIVER_ATTR(dev_init,S_IWUGO | S_IRUGO, NULL, pa22x_store_dev_init);
static DRIVER_ATTR(test, S_IWUGO | S_IRUGO,pa22x_show_test, pa22x_store_test);
static DRIVER_ATTR(irq_test, S_IWUGO | S_IRUGO,pa22x_show_irq_test, pa22x_store_irq_test);

/*add by yangguibin start*/
static DRIVER_ATTR(chip_name,     S_IWUGO | S_IRUGO, pa22x_show_chip_name, NULL);
static DRIVER_ATTR(prox_uncover_max,     S_IWUGO | S_IRUGO, pa22x_show_prox_uncover_max, NULL);
static DRIVER_ATTR(prox_offset_cal,     S_IWUGO | S_IRUGO, pa22x_show_prox_offset_cal, NULL);
static DRIVER_ATTR(prox_thres_min,     S_IWUGO | S_IRUGO, pa22x_show_prox_thres_min, NULL);
static DRIVER_ATTR(prox_thres_max,     S_IWUGO | S_IRUGO, pa22x_show_prox_thres_max, NULL);
static DRIVER_ATTR(prox_thres,     S_IWUGO | S_IRUGO, pa22x_show_prox_thres, NULL);

/*add for bh1745*/
static DRIVER_ATTR(light_value,     S_IWUGO | S_IRUGO, bh1745_show_light_value, NULL);//yangkui
static DRIVER_ATTR(light_chip_name,     S_IWUGO | S_IRUGO, bh1745_show_light_chip_name, NULL);
static DRIVER_ATTR(tp_cfg,     S_IWUGO | S_IRUGO, bh1745_show_tp_cfg_color, bh1745_set_light_calibration);
static DRIVER_ATTR(fac_calibrate,     S_IWUGO | S_IRUGO, bh1745_show_rgb_factory_cal, bh1745_store_rgb_factory_cal);
/*add by yangguibin end*/
...
static int pa22x_check_intr(struct i2c_client *client) 
{
	...
	else if (psdata >= obj->ps_thrd_high)
	{
		...
		for (i=0; i<ps_ary_size; i++)
		{
			hwmsen_read_byte(client, REG_PS_DATA, &ps_seq_near[i]);
			if (i > 0)
...
static void pa22x_eint_work(struct work_struct *work)
{
	...
	hwm_sensor_data sensor_data_ps, sensor_data_als;
	int res = 0;
	u8 regdata = 0;
