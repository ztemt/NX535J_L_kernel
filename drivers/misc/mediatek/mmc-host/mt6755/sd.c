...
static void msdc_enable_cd_irq(struct msdc_host *host, int enable)
{
	...
	if (enable) {
		...
		printk(KERN_ERR"alexander:gpiopin is %d,debounce is %d\n",gpiopin,debounce);
		...
		if (hw->cd_level) {
			...
			//flags = IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING;//xiaofeng modify LOW to mix
		}
		/* request irq for eint (either way) */
		printk(KERN_ERR"alexander:cd_irq is %d,flags is %lu\n",cd_irq,flags);
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (hw->flags & MSDC_CD_PIN_EN) {
		/* set for card */
		printk(KERN_ERR"alexander:MSDC_CD_PIN_EN is 1 ,so into msdc_enable_cd_irq flow in %s\n",__func__);
