...
#define NUBIA_TEEI_DEBUG 0
#if NUBIA_TEEI_DEBUG
#define NUBIA_TEEI_DBG(fmt, args...) printk("\033[;34m[TEEI][TZDriver]"fmt"\033[0m", ##args)
#else
#define NUBIA_TEEI_DBG(fmt, args...)
#endif
#define NUBIA_TEEI_MSG(fmt, args...) printk("\033[;34m[TEEI][TZDriver]"fmt"\033[0m", ##args)
...
void nt_sched_t_call(void)
{
	...
	int retVal = 0;
        ...
        if (retVal != 0) {
                NUBIA_TEEI_MSG("[%s][%d] add_work_entry function failed!\n", __func__, __LINE__);
        }
#endif
	...
	while (1) {
		...
		if (teei_config_flag == 1) {
			...
			if (retVal == -ERESTARTSYS) {
				NUBIA_TEEI_MSG("[%s][%d]*********down &global_down_lock failed *****************\n", __func__, __LINE__ );
				continue;
			}
		}
		...
		if (retVal != 0) {
			NUBIA_TEEI_MSG("[%s][%d]*********down &smc_lock failed *****************\n", __func__, __LINE__ );
		...
		struct semaphore *psema)
{
	...
	if (ret) {
		NUBIA_TEEI_MSG("[%s][%d] smc_call returns error!\n", __func__, __LINE__);
		goto out;
	}
		...
		struct semaphore *psema)
{
	...
	if (local_smc_cmd == NULL) {
		NUBIA_TEEI_MSG("[%s][%d] tz_malloc_shared_mem failed!\n", __func__, __LINE__);
		return -1;
	}
		...
		struct teei_session *temp_ses)
{
	...
	NUBIA_TEEI_DBG("======== ses_close->session_id = %d =========\n", ses_close->session_id);
...
static int teei_client_session_init(void *private_data, void *argp)
{
	...
	if (copy_from_user(&ses_init, argp, sizeof(ses_init))) {
		NUBIA_TEEI_MSG("[%s][%d] copy from user failed!\n", __func__, __LINE__);
		return  -EFAULT;
	}
	...
	if (!ctx_found) {
		NUBIA_TEEI_MSG("[%s][%d] can't find context.\n", __func__, __LINE__);
		return -EINVAL;
	}
	...
	if (ses_new == NULL) {
		NUBIA_TEEI_MSG("[%s][%d] tz_malloc failed.\n", __func__, __LINE__);
		return -ENOMEM;
	}
...
static int teei_client_session_open(void *private_data, void *argp)
{
	...
	if (sess_found == 0) {
		NUBIA_TEEI_MSG("[%s][%d] can't find session!\n", __func__, __LINE__);
		tz_free_shared_mem(ses_open, sizeof(struct ser_ses_id));
		return -EINVAL;
	}
	...
	if (retVal != SMC_SUCCESS) {
		NUBIA_TEEI_MSG("[%s][%d] open session smc error!\n", __func__, __LINE__);
		goto clean_hdr_buf;
	}

	if (ses_open->session_id == -1)
		NUBIA_TEEI_MSG("[%s][%d] invalid session id!\n", __func__, __LINE__);
...
static int teei_client_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (alloc_addr == 0) {
		NUBIA_TEEI_MSG("[%s][%d] get free pages failed!\n", __func__, __LINE__);
	...
	if (retVal) {
		NUBIA_TEEI_MSG("[%s][%d] remap_pfn_range failed!\n", __func__, __LINE__);
		kfree(share_mem_entry);
...
static int teei_client_send_cmd(void *private_data, void *argp)
{
	...
	if (ctx_found == 0) {
		NUBIA_TEEI_MSG("[%s][%d] can't find context data!\n", __func__, __LINE__);
		return -EINVAL;
	}
	...
	if (sess_found == 0) {
		NUBIA_TEEI_MSG("[%s][%d] can't find session data!\n", __func__, __LINE__);
		return -EINVAL;
	}
	...
	if (enc_found == 0) {
		NUBIA_TEEI_MSG("[%s][%d] can't find encode data!\n", __func__, __LINE__);
		return -EINVAL;
	}
	...
	if (retVal != SMC_SUCCESS)
		NUBIA_TEEI_MSG("[%s][%d] send cmd secure call failed!\n", __func__, __LINE__);

	if (copy_to_user(argp, &enc, sizeof(enc))) {
		NUBIA_TEEI_MSG("[%s][%d] copy to user failed!\n", __func__, __LINE__);
		return -EFAULT;
	}
...
static int teei_client_context_init(void *private_data, void *argp)
{
	...
	NUBIA_TEEI_DBG("[%s][%d] context name = %s.\n ", __func__, __LINE__, name);
...
static int teei_client_operation_release(void *private_data, void *argp)
{
	...
	if (ctx_found == 0) {
		NUBIA_TEEI_MSG("[%s][%d] ctx_found failed!\n", __func__, __LINE__);
		return -EINVAL;
	}
	...
	if (session_found == 0) {
		NUBIA_TEEI_MSG("[%s][%d] session_found failed!\n", __func__, __LINE__);
		return -EINVAL;
	}
	...
	if (enc_found == 0) {
		NUBIA_TEEI_MSG("[%s][%d] enc_found failed!\n", __func__, __LINE__);
		return -EINVAL;
	} else {
		...
		struct teei_session **psession)
{
	...
	if (!session_found) {
		NUBIA_TEEI_MSG("[%s][%d] session (ID: %x) not found!\n", __func__, __LINE__, enc->session_id);
		return -EINVAL;
	}
...
int teei_client_encode_uint32(void *private_data, void *argp)
{
	...
	if (enc.param_type == TEEIC_PARAM_IN) {
		if (enc_context->ker_req_data_addr == NULL) {
			...
			if (enc_context->ker_req_data_addr == NULL) {
				NUBIA_TEEI_MSG("[%s][%d] enc_context->ker_req_data_addr is NULL!\n", __func__, __LINE__);
				retVal = -ENOMEM;
				goto ret_encode_u32;
			}
		}
		...
		if (!enc_context->ker_res_data_addr) {
			...
			if (!enc_context->ker_res_data_addr) {
				NUBIA_TEEI_MSG("[%s][%d] enc_context->ker_res_data_addr is NULL\n", __func__, __LINE__);
				retVal = -ENOMEM;
				goto ret_encode_u32;
			}
		}
		if ((enc_context->enc_res_offset + sizeof(u32) <= TEEI_1K_SIZE) &&
			...
			if (enc.data != NULL) {
				...
				//enc.data �?2位的时候，可以直接赋值，但是不能直接寻址，因为这个变量的高位�?用FFFF FFFf填充�?				enc_context->meta[enc_context->enc_res_pos].usr_addr = (unsigned int)enc.data;
			} else {
...
int teei_client_encode_mem_ref_64bit(void *private_data, void *argp)
{
	...
	if (shared_mem_found == 0) {
        ...
        //printk("[%s][%d]test encode mem_ref temp_shared_mem->index100 u32 is ( %x) !\n", __func__, __LINE__, (unsigned int )(temp_shared_mem->index));
					//temp_addr = temp_shared_mem->index;//如果�?2位API定义32位API进行比较enc.data
	...
	if (!shared_mem_found) {
        NUBIA_TEEI_MSG("[%s][%d]shared_mem_found!\n", __func__, __LINE__);
		retVal = -EINVAL;
		goto return_func;
	}
...
ret_encode_array:
    NUBIA_TEEI_DBG("[%s][%d] teei_client_encode_mem_ref end ret_encode_array.\n", __func__, __LINE__);
	if (copy_to_user(argp, &enc, sizeof(enc))) {
		NUBIA_TEEI_MSG("[%s][%d] copy from user failed!\n", __func__, __LINE__);
		return -EFAULT;
	}
...
return_func:
	NUBIA_TEEI_MSG("[%s][%d] teei_client_encode_mem_ref end return_func.\n", __func__, __LINE__);
	return retVal;
}
		...
		struct teei_encode **pdec_context)
{
	...
	if (0 == session_found) {
		NUBIA_TEEI_MSG("[%s][%d] session not found!\n", __func__, __LINE__);
		return -EINVAL;
	}
	...
	/* print_context(); */
	if (0 == enc_found) {
		NUBIA_TEEI_MSG("[%s][%d] encode[%x] not found!\n", __func__, __LINE__, dec->encode_id);
		return -EINVAL;
	}
...
static int teei_client_decode_uint32(void *private_data, void *argp)
{
	...
	if (retVal != 0) {
		NUBIA_TEEI_MSG("[%s][%d] teei_client_prepare_decode failed!\n", __func__, __LINE__);
		goto return_func;
	}
	...
	if ((dec_context->dec_res_pos <= dec_context->enc_res_pos) &&
		...
		if (dec_context->meta[dec_context->dec_res_pos].len >=
			...
			if (copy_to_user(dec.data, (char *)dec_context->ker_res_data_addr + dec_context->dec_offset,
						dec_context->meta[dec_context->dec_res_pos].ret_len)) {
				NUBIA_TEEI_MSG("[%s][%d] copy from user failed while copying array!\n", __func__, __LINE__);
				retVal = -EFAULT;
				goto return_func;
			}
		} else {

			NUBIA_TEEI_MSG("[%s][%d] buffer length is small. Length required %x and supplied length %x,pos %x ",
					__func__, __LINE__,
					dec_context->meta[dec_context->dec_res_pos].ret_len,
		...
		if (dec_context->meta[dec_context->dec_res_pos].len >=
			...
			NUBIA_TEEI_MSG("[%s][%d] buffer length is small. Length required %x and supplied length %x",
					__func__, __LINE__,
					dec_context->meta[dec_context->dec_res_pos].ret_len,
					dec_context->meta[dec_context->dec_res_pos].len);
		...
		NUBIA_TEEI_MSG("[%s][%d] invalid data type or decoder at wrong position!\n", __func__, __LINE__);
		retVal = -EINVAL;
		goto return_func;
	}
...
static long teei_client_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
	...
	if (teei_config_flag == 0) {
		NUBIA_TEEI_MSG("Error: soter is NOT ready, Can not support IOCTL!\n");
		return -ECANCELED;
	}
	down(&api_lock);
	mutex_lock(&pm_mutex);
	...
	case TEEI_CLIENT_IOCTL_INITCONTEXT_REQ:
			...
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_INITCONTEXT beginning .....\n", __func__, __LINE__);
			retVal = teei_client_context_init(file->private_data, argp);
			if (retVal != 0)
				NUBIA_TEEI_MSG("[%s][%d] failed init context %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_INITCONTEXT end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_CLOSECONTEXT_REQ:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_CLOSECONTEXT beginning .....\n", __func__, __LINE__);
			retVal = teei_client_context_close(file->private_data, argp);
			if (retVal != 0)
				NUBIA_TEEI_MSG("[%s][%d] failed close context: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_CLOSECONTEXT end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_SES_INIT_REQ:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SES_INIT beginning .....\n", __func__, __LINE__);
			retVal = teei_client_session_init(file->private_data, argp);
			if (retVal != 0)
				NUBIA_TEEI_MSG("[%s][%d] failed session init: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SES_INIT end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_SES_OPEN_REQ:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SES_OPEN beginning .....\n", __func__, __LINE__);
			retVal = teei_client_session_open(file->private_data, argp);
			if (retVal != 0)
				NUBIA_TEEI_MSG("[%s][%d] failed session open: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SES_OPEN end .....\n", __func__, __LINE__);
			break;


	case TEEI_CLIENT_IOCTL_SES_CLOSE_REQ:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SES_CLOSE beginning .....\n", __func__, __LINE__);
			retVal = teei_client_session_close(file->private_data, argp);
			if (retVal != 0)
				NUBIA_TEEI_MSG("[%s][%d] failed session close: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SES_CLOSE end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_OPERATION_RELEASE:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_OPERATION_RELEASE beginning .....\n", __func__, __LINE__);
			retVal = teei_client_operation_release(file->private_data, argp);
			if (retVal != 0)
				NUBIA_TEEI_MSG("[%s][%d] failed operation release: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_OPERATION_RELEASE end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_SEND_CMD_REQ:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SEND_CMD beginning .....\n", __func__, __LINE__);
			retVal = teei_client_send_cmd(file->private_data, argp);
			if (retVal != 0)
				NUBIA_TEEI_MSG("[%s][%d] failed send cmd: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SEND_CMD end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_GET_DECODE_TYPE:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_GET_DECODE beginning .....\n", __func__, __LINE__);
			retVal = teei_client_get_decode_type(file->private_data, argp);
			if (retVal != 0)
				NUBIA_TEEI_MSG("[%s][%d] failed decode cmd: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_GET_DECODE end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_ENC_UINT32:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_ENC_UINT32 beginning .....\n", __func__, __LINE__);
			retVal = teei_client_encode_uint32(file->private_data, argp);
			if (retVal != 0)
				NUBIA_TEEI_MSG("[%s][%d] failed teei_client_encode_cmd: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_ENC_UINT32 end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_DEC_UINT32:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_DEC_UINT32 beginning .....\n", __func__, __LINE__);
			retVal = teei_client_decode_uint32(file->private_data, argp);
			if (retVal != 0)
				NUBIA_TEEI_MSG("[%s][%d] failed teei_client_decode_cmd: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_DEC_UINT32 end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_ENC_ARRAY:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_ENC_ARRAY beginning .....\n", __func__, __LINE__);
			retVal = teei_client_encode_array(file->private_data, argp);
			if (retVal != 0)
				NUBIA_TEEI_MSG("[%s][%d] failed teei_client_encode_cmd: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_ENC_ARRAY end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_DEC_ARRAY_SPACE:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_DEC_ARRAY_SPACE beginning .....\n", __func__, __LINE__);
			retVal = teei_client_decode_array_space(file->private_data, argp);
			if (retVal != 0)
				printk("[%s][%d] failed teei_client_decode_cmd: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_DEC_ARRAY_SPACE end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_ENC_MEM_REF:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_DEC_MEM_REF beginning .....\n", __func__, __LINE__);
			retVal = teei_client_encode_mem_ref(file->private_data, argp);
			if (retVal != 0)
				printk("[%s][%d] failed teei_client_encode_cmd: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_DEC_MEM_REF end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_ENC_ARRAY_SPACE:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_ENC_ARRAY_SPACE beginning .....\n", __func__, __LINE__);
			retVal = teei_client_encode_mem_ref(file->private_data, argp);
			if (retVal != 0)
				printk("[%s][%d] failed teei_client_encode_cmd: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_ENC_ARRAY_SPACE end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_SHR_MEM_ALLOCATE_REQ:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SHR_MEM_ALLOCATE beginning .....\n", __func__, __LINE__);
			retVal = teei_client_shared_mem_alloc(file->private_data, argp);
			if (retVal != 0)
				printk("[%s][%d] failed teei_client_shared_mem_alloc: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SHR_MEM_ALLOCATE end .....\n", __func__, __LINE__);
			break;

	case TEEI_CLIENT_IOCTL_SHR_MEM_FREE_REQ:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SHR_MEM_FREE beginning .....\n", __func__, __LINE__);
			retVal = teei_client_shared_mem_free(file->private_data, argp);
			if (retVal != 0)
				printk("[%s][%d] failed teei_client_shared_mem_free: %x.\n", __func__, __LINE__, retVal);
			NUBIA_TEEI_DBG("[%s][%d] TEEI_CLIENT_IOCTL_SHR_MEM_FREE end .....\n", __func__, __LINE__);
			break;

	case TEEI_GET_TEEI_CONFIG_STAT:

			NUBIA_TEEI_DBG("[%s][%d] TEEI_GET_TEEI_CONFIG_STAT beginning .....\n", __func__, __LINE__);
			retVal = teei_config_flag;
			NUBIA_TEEI_DBG("[%s][%d] TEEI_GET_TEEI_CONFIG_STAT end .....\n", __func__, __LINE__);
			break;

	default:
			NUBIA_TEEI_DBG("[%s][%d] command not found!\n", __func__, __LINE__);
			retVal = -EINVAL;
	}
...
int register_sched_irq_handler(void)
{
	...
	int retVal = 0;
        ...
        if (retVal)
                ...
                NUBIA_TEEI_DBG("[CONFIG_OF] [%s] request irq [ %d ] OK.\n", __func__, irq_num);
	...
	if (retVal)
		...
		NUBIA_TEEI_DBG("request irq [ %d ] OK.\n", SCHED_IRQ);
#endif
...
static irqreturn_t nt_boot_irq_handler(void)
{
	int cpu_id = raw_smp_processor_id();
	if (boot_soter_flag == START_STATUS) {
		NUBIA_TEEI_DBG("boot irq  handler if\n");
		boot_soter_flag = END_STATUS;
		up(&smc_lock);
		up(&(boot_sema));
		return IRQ_HANDLED;
	} else {
		NUBIA_TEEI_DBG("boot irq hanler else\n");
...
void tlog_func(struct work_struct *entry)
{
	...
	NUBIA_TEEI_DBG("TLOG %s", (char *)(ts->context));
	ts->valid = TLOG_UNUSE;
	return;
}
...
static void boot_stage1(unsigned long vir_address)
{
	...
	get_online_cpus();
	cpu_id = get_current_cpuid();
	NUBIA_TEEI_DBG("current cpu id [%d]\n", cpu_id);
...
static int init_teei_framework(void)
{
	...
	NUBIA_TEEI_DBG("[%s][%d]\n", __func__, __LINE__);
	NUBIA_TEEI_DBG("[%s][%d] VFS_SIZE = %d, SZ_4K = %d\n", __func__, __LINE__, VFS_SIZE, SZ_4K);
	NUBIA_TEEI_DBG("[%s][%d] ROUND_UP(VFS_SIZE, SZ_4K) = %d\n", __func__, __LINE__, ROUND_UP(VFS_SIZE, SZ_4K));
	NUBIA_TEEI_DBG("[%s][%d] get_order(ROUND_UP(VFS_SIZE, SZ_4K)) = %d\n", __func__, __LINE__, get_order(ROUND_UP(VFS_SIZE, SZ_4K)));
	...
	**printk("[%s][%d] load Soter services successfully.\n", __func__, __LINE__);*/
	...
	NUBIA_TEEI_DBG("[%s][%d] begin to create the command buffer!\n", __func__, __LINE__);
	retVal = create_cmd_buff();
	if (retVal < 0) {
		printk("[%s][%d] create_cmd_buff failed !\n", __func__, __LINE__);
		return retVal;
	}
	NUBIA_TEEI_DBG("[%s][%d] end of creating the command buffer!\n", __func__, __LINE__);

	NUBIA_TEEI_DBG("[%s][%d] begin to load Soter services.\n", __func__, __LINE__);
	switch_to_t_os_stages2();
	NUBIA_TEEI_DBG("[%s][%d] load Soter services successfully.\n", __func__, __LINE__);
	...
	NUBIA_TEEI_DBG("[%s][%d] begin to init daulOS services.\n", __func__, __LINE__);
	retVal = teei_service_init();
	if (retVal == -1)
		return -1;

	NUBIA_TEEI_DBG("[%s][%d] init daulOS services successfully.\n", __func__, __LINE__);

	NUBIA_TEEI_DBG("[%s][%d] begin to load TEEs.\n", __func__, __LINE__);
	t_os_load_image();
	if (soter_error_flag == 1)
		return -1;

	NUBIA_TEEI_DBG("[%s][%d] load TEEs successfully.\n", __func__, __LINE__);
		...
		unsigned long action, void *hcpu)
{
	...
	case CPU_DOWN_PREPARE_FROZEN:
			if (cpu == sched_cpu) {
				NUBIA_TEEI_DBG("cpu down prepare ************************\n");
				...
				if (retVal == 1) {
					...
					for_each_online_cpu(i) {
						NUBIA_TEEI_DBG("current on line cpu [%d]\n", i);
						if (i == cpu) {
							continue;
						}
						current_cpu_id = i;
...
#if 1
					NUBIA_TEEI_DBG("[%s][%d]brefore cpumask set cpu\n", __func__, __LINE__);
					...
					NUBIA_TEEI_DBG("current cpu id  \n");
					nt_sched_core(teei_cpu_id[current_cpu_id], teei_cpu_id[cpu], 0);
					NUBIA_TEEI_DBG("change cpu id = [%d]\n", current_cpu_id);
				}
			}
			break;
	...
	case CPU_DEAD_FROZEN:
			if (cpu_notify_flag == 1) {
				NUBIA_TEEI_DBG("cpu down success ***********************\n");
				up(&smc_lock);
				cpu_notify_flag = 0;
			}
			break;
...
static int teei_client_init(void)
{
	...
	for_each_online_cpu(i) {
		current_cpu_id = i;
		NUBIA_TEEI_DBG("init stage : current_cpu_id = %d\n", current_cpu_id);
	}
	NUBIA_TEEI_DBG("begin to create sub_thread.\n");
#if 0
...
#endif
	NUBIA_TEEI_DBG("create the sub_thread successfully!\n");
#if 0
	...
	NUBIA_TEEI_DBG("after  register cpu notify\n");
	teei_config_init();
