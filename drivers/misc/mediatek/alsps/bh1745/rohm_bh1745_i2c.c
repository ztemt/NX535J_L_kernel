...
static COLOR_T bh1745_get_color_type(void)
{
    ...
    res = rgb_bh1745_file_read(COLOR_CONFIG_PATH, &cfg, sizeof(cfg));
...
ssize_t bh1745_show_light_value(struct device_driver *ddri, char *buf)
{
	ssize_t res;
	int light_value = ALSPS_INVALID_VALUE; 
	...
	light_value =  bh1745_calculate_light(data, gain, time);

	light_value = light_value * SCALE_FACTOR(bh1745_get_base_lux(), bh1745_get_curr_lux());
		
	BH1745_ERR("bh1745 light_value = %d\n", light_value);
	res = sprintf(buf, "%d", light_value); 
	return res;
}
...
ssize_t bh1745_set_light_calibration(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    if (tmp == 0x80){
	 if (value == 0x80 || value == 0x81 || value == 0x82)
         ...
         res = rgb_bh1745_file_write(COLOR_CONFIG_PATH, (char *)&value, sizeof(value));
	 if (res < 0) {
...
ssize_t bh1745_show_rgb_factory_cal(struct device_driver *ddri, char *buf)
{
    ...
    res = rgb_bh1745_file_read(RGBC_CAL_PATH, &rgb_cfg, sizeof(rgb_cfg));
...
ssize_t bh1745_store_rgb_factory_cal(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    res = rgb_bh1745_file_write(RGBC_CAL_PATH, &rgb_cfg, sizeof(rgb_cfg));
	if (res < 0){
        ALSPS_ERR("rgb_bh1745_file_write write value failed.\n");
	    return res;
	}
