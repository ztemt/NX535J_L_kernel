...
#include "imx258mipiraw_main_537_Sensor.h"

/****************************Modify following Strings for debug****************************/
#define PFX "IMX258_main_537_camera_sensor"
...
static kal_bool Is_longExp=0;  //tanyijun add 

static BYTE imx258_SPC_data[252]={0};

extern void read_imx258_SPC( BYTE* data );
extern void read_imx258_DCC( kal_uint16 addr,BYTE* data, kal_uint32 size);

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX258_MAIN_537_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =
{
 { 4208, 3120,	  4,	0, 4200, 3120, 2100,  1560, 0000, 0000, 2100,  1560,	  0,	0, 2100, 1560}, // Preview
	 ...
	 { 4208, 3120,	  8,  	0, 4192, 3120, 1280,  720 , 0000, 0000, 1280,	720,	  0,	0, 1280,  720}, //hight speed video
	 { 4208, 3120,	  4,	0, 4200, 3120, 2100,  1560, 0000, 0000, 2100,  1560,	  0,	0, 2100, 1560},// slim video
 //ZTEMT:wangkai add for night_mode----Start
 { 4208, 3120,	  4,	0, 4200, 3120, 4200,  3120, 0000, 0000, 4200,  3120,	  0,	0, 4200, 3120}, // Custom1 (defaultuse preview)
 { 4208, 3120,	  4,	0, 4200, 3120, 4200,  3120, 0000, 0000, 4200,  3120,	  0,	0, 4200, 3120}, // Custom2
 //ZTEMT:wangkai add for night_mode----End
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info_Binning[10] =
	{{ 4208, 3120,	  4,	0, 4200, 3120, 2100,  1560, 0000, 0000, 2100,  1560,	  0,	0, 2100, 1560}, // Preview
	 ...
	 { 4208, 3120,	  8,  	0, 4192, 3120, 1280,  720 , 0000, 0000, 1280,	720,	  0,	0, 1280,  720},
	 { 4208, 3120,	  4,	0, 4200, 3120, 2100,  1560, 0000, 0000, 2100,  1560,	  0,	0, 2100, 1560}};// slim video
...
#if 0
static kal_uint32 imx258_ATR(UINT16 DarkLimit, UINT16 OverExp)
{
/*
...
static void imx258_apply_SPC(void)
{
	int i;

	if(read_spc_flag == FALSE)
	{
		read_imx258_SPC(imx258_SPC_data);
		read_spc_flag = TRUE;
		return;
	}
	...
	for(i=0; i<63; i++)
	{
		write_cmos_sensor(0xD04C+i, imx258_SPC_data[i]);
		LOG_INF("SPC_Data[%d] = %d\n", i, imx258_SPC_data[i]);
		//LOG_INF("read_SPC_Data[%d] = %d\n", i, read_cmos_sensor(0xD04C+i));
	}
	for(i=0; i<63; i++)
	{
		write_cmos_sensor(0xD08C+i, imx258_SPC_data[i+63]);
		LOG_INF("SPC_Data[%d] = %d\n", i+63, imx258_SPC_data[i+63]);
...
static void set_shutter(kal_uint32 shutter)
{
    ...
    kal_uint64 shutter_time = 0;
    kal_uint16 shutter_timeS = 0;

    kal_uint32 line_fix = 0;
    kal_uint32 line = 0;
    kal_uint32 fl_lines = 0;
    kal_uint16 times_count = 0;
    kal_uint16 temp = 1;
    kal_uint8 i = 0;
	
    static  KERNEL_SENSOR_AE_STATUS_ENUM pre_ae_status = SENSOR_AE_STATUS_IS_NORMAL_SHUTTER;
    static  KERNEL_SENSOR_AE_STATUS_ENUM cur_ae_status = SENSOR_AE_STATUS_IS_NORMAL_SHUTTER;
    spin_lock_irqsave(&imgsensor_drv_lock, flags);
    imgsensor.shutter = shutter;
    spin_unlock_irqrestore(&imgsensor_drv_lock, flags);
    LOG_INF("tanSS  shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
    ...
    if(shutter>0xFFFF)
           Is_longExp=1;
    LOG_INF("tanSS  Is_longExp =%d, pre_ae_status =%d\n", Is_longExp,pre_ae_status);	
   if(Is_longExp)
    { 
        cur_ae_status = SENSOR_AE_STATUS_IS_SLOW_SHUTTER;
        // m_LineTimeInus[0] = (MUINT32)(((MUINT64)pFeaturePara16[0]*1000000 + ((FeaturePara32/1000)-1))/(FeaturePara32/1000)); 
         shutter_time =(kal_uint64) shutter;
         shutter_time = shutter_time * 10325;//modified by houyujun
	    // 四舍五入 取整
	     shutter_timeS = ((shutter_time +500000000) /1000000000);

	     LOG_INF("tanSS shutter = %d shutter_time = %d  shutter_timeS = %d\n", shutter,shutter_time,shutter_timeS);
           
	     write_cmos_sensor(0x0100,0x00);	
	     write_cmos_sensor(0x0104, 0x01);   
         if(shutter_timeS > 0 && shutter_timeS < 112)
          {    
		 // set clock plck = ( (24M*50)/(5*2*4) ) * 4(lane) = 120 000 000
                 write_cmos_sensor(0x0301, 0x05);
                 write_cmos_sensor(0x0303, 0x02);
                 write_cmos_sensor(0x0305, 0x04);
                 write_cmos_sensor(0x0306, 0x00);
                 write_cmos_sensor(0x0307, 0x32); //50
		        ...
		        // set LINE_LENGTH_PCK = 5352
		   write_cmos_sensor(0x0342, 0x14);
                 write_cmos_sensor(0x0343, 0xE8);
                 // set  FRM_LENGH_LINES = 19500
		  write_cmos_sensor(0x0340, 0x4C);
                 ...
                 write_cmos_sensor(0x0823, 0x00);
                   ///120 000 000 /5352/19500=1.14
		          /// 1s 5352*19500*1.14/5352 = 22230	  
		    line_fix = 22230;

	      }
            else if(shutter_timeS >= 112 && shutter_timeS < 129)
            {
               // set clock plck = ( (24M*50)/(5*2*4) ) * 4(lane) = 120 000 000
                 write_cmos_sensor(0x0301, 0x05);
                 write_cmos_sensor(0x0303, 0x02);
		  ...
		  // set FRM_LENGH_LINES = 22200
		   write_cmos_sensor(0x0340, 0x57);
                 write_cmos_sensor(0x0341, 0x95);				 
                   ///120 000 000 /5352/22200=1
		    /// 1s 5352/22200/5 = 22200	
                line_fix = 22421;
            }
            else if(shutter_timeS >= 129 && shutter_timeS < 257)
            {
               // set clock plck = ( (24M*50)/(5*2*4) ) * 4(lane) = 120 000 000
                 write_cmos_sensor(0x0301, 0x05);
                 write_cmos_sensor(0x0303, 0x02);
		  ...
		  // set LINE_LENGTH_PCK = 5352
		   write_cmos_sensor(0x0342, 0x14);
                 write_cmos_sensor(0x0343, 0xE8);
                 //120 000 000/5352/44400 = 0.5  
		  // set  FRM_LENGH_LINES = 44400
		   write_cmos_sensor(0x0340, 0xAD);
                 write_cmos_sensor(0x0341, 0x70);				 

		    /// 1s 44400*0.5 = 22200	
                line_fix = 22200;
            }
            else if(shutter_timeS >= 257 && shutter_timeS < 513)
            {
               // set clock plck = ( (24M*50)/(5*2*4) ) * 4(lane) = 120 000 000
                 write_cmos_sensor(0x0301, 0x05);
                 write_cmos_sensor(0x0303, 0x02);
                 ...
                 write_cmos_sensor(0x0822, 0x00);
                 write_cmos_sensor(0x0823, 0x00);
		  // set LINE_LENGTH_PCK = 10000
		   write_cmos_sensor(0x0342, 0x27);
                 write_cmos_sensor(0x0343, 0x10);
                 //120 000 000/10000/47000 = 0.25  
		  // set  FRM_LENGH_LINES = 47000
		   write_cmos_sensor(0x0340, 0xB7);
            ...
            else if(shutter_timeS >= 513 && shutter_timeS < 1025)
            {
               // set clock plck = ( (24M*50)/(5*2*4) ) * 4(lane) = 120 000 000
                 write_cmos_sensor(0x0301, 0x05);
                 write_cmos_sensor(0x0303, 0x02);
		  ...
		  // set LINE_LENGTH_PCK = 20000
		   write_cmos_sensor(0x0342, 0x4E);
		   write_cmos_sensor(0x0343, 0x20);
                 //120 0S00 000/20000/47000 = 0.125  
		  // set  FRM_LENGH_LINES = 47000
		   write_cmos_sensor(0x0340, 0xB7);
                 write_cmos_sensor(0x0341, 0x98);				 
            ...
            else if(shutter_timeS >= 1025 && shutter_timeS < 1281)
            {
               // set clock plck = ( (24M*50)/(5*2*4) ) * 4(lane) = 120 000 000
                 write_cmos_sensor(0x0301, 0x05);
                 write_cmos_sensor(0x0303, 0x02);
		  ...
		  // set LINE_LENGTH_PCK = 25000
                 write_cmos_sensor(0x0342, 0x61);
                 write_cmos_sensor(0x0343, 0xA8);
                 //201 600 000/25000/47000 = 0.1  
		  // set  FRM_LENGH_LINES = 47000
		   write_cmos_sensor(0x0340, 0xB7);
                 write_cmos_sensor(0x0341, 0x98);			 
            ...
            else if(shutter_timeS >= 1281 && shutter_timeS < 2290)
            {
               // set clock plck = ( (24M*50)/(5*2*4) ) * 4(lane) = 120 000 000
                 write_cmos_sensor(0x0301, 0x05);
                 write_cmos_sensor(0x0303, 0x02);
		  ...
		  // set LINE_LENGTH_PCK = 45000
                 write_cmos_sensor(0x0342, 0xAF);
                 write_cmos_sensor(0x0343, 0xC8);
                 //120 000 000/45000/47000 = 0.0559  
		  // set  FRM_LENGH_LINES = 47000
		   write_cmos_sensor(0x0340, 0xB7);
                 write_cmos_sensor(0x0341, 0x98);				 

		    /// 1s 47000*0.0559 = 2627       
		    line_fix = 2627;
             //   line_fix = 3662.7907;
            }
            else
	 ...
	 line = line_fix * shutter_timeS;
	 fl_lines = line + 10;

	 while(fl_lines > 65535)
	 {
            fl_lines /= 2;
            times_count = times_count + 1;
        }

       // temp = pow(2,times_count);
	 if(times_count > 0)
	 {
	    // temp =1;
	 ...
	 LOG_INF("tanSS times_count = %d temp = %d  line = %d\n", times_count,temp,line);
		
        if (times_count >= 1) {
            fl_lines = line + 10;
        }

        write_cmos_sensor(0x0340, (fl_lines & 0xFF00) >> 8);
        write_cmos_sensor(0x0341, fl_lines & 0xFF);

        write_cmos_sensor(0x0202, (line >> 8) & 0xFF);
        write_cmos_sensor(0x0203, line  & 0xFF);	

	 write_cmos_sensor(0x3002, times_count & 0xFF);	
...
if(pre_ae_status == SENSOR_AE_STATUS_IS_SLOW_SHUTTER)
  {
  
    shutter = (shutter < imgsensor_info.min_shutter) ? imgsensor_info.min_shutter : shutter;
    shutter = (shutter > (imgsensor_info.max_frame_length - imgsensor_info.margin)) ? (imgsensor_info.max_frame_length - imgsensor_info.margin) : shutter;

    cur_ae_status = SENSOR_AE_STATUS_IS_NORMAL_SHUTTER;
    ...
    if (imgsensor.autoflicker_en) {
		...
		write_cmos_sensor(0x0310,0x00);
		  ...
		  // set LINE_LENGTH_PCK = 27520
		//   write_cmos_sensor(0x0342, 0x6B);
      // write_cmos_sensor(0x0343, 0x80);
	   ...
	   /*************data rateing setting*********/ 								  
		...
		write_cmos_sensor(0x0823,0x00);	
		write_cmos_sensor(0x0104, 0x00); 
                 //201 600 000/27520/65535 = 0.055  
		  // set  FRM_LENGH_LINES = 65535
       ...
       //  	write_cmos_sensor(0x0350, 0x01);
             write_cmos_sensor(0x0100,0x01);	
     //    reg202 = read_cmos_sensor(0x202);
      //   reg203 = read_cmos_sensor(0x203);
	
            LOG_INF("tanAE  Exit! shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
    //    LOG_INF("tanAE  Exit! reg202 =%d, reg203 =%d\n", reg202,reg203);	
     //   LOG_INF("tanAE  Exit! regshutter = 0x%x \n",( reg202) <<8 |reg203);	

}
else if(pre_ae_status == SENSOR_AE_STATUS_IS_NORMAL_SHUTTER)
{
    cur_ae_status = SENSOR_AE_STATUS_IS_NORMAL_SHUTTER;
    shutter = (shutter < imgsensor_info.min_shutter) ? imgsensor_info.min_shutter : shutter;
    shutter = (shutter > (imgsensor_info.max_frame_length - imgsensor_info.margin)) ? (imgsensor_info.max_frame_length - imgsensor_info.margin) : shutter;
...
write_cmos_sensor(0x0341,	0x24);
	...
	write_cmos_sensor(0x0408,0x00);
write_cmos_sensor(0x0409,	0xB2);
	write_cmos_sensor(0x040A,0x00);
write_cmos_sensor(0x040B,	0x10);
write_cmos_sensor(0x040C,	0x0F);
write_cmos_sensor(0x040D,	0x0C);
...
static kal_uint32 imx258_awb_gain(SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
    
    UINT32 rgain_32, grgain_32, gbgain_32, bgain_32;

LOG_INF("imx258_awb_gain\n");
    ...
    LOG_INF("[imx258_awb_gain] ABS_GAIN_GR:%d, grgain_32:%d\n", pSetSensorAWB->ABS_GAIN_GR, grgain_32);
    LOG_INF("[imx258_awb_gain] ABS_GAIN_R:%d, rgain_32:%d\n", pSetSensorAWB->ABS_GAIN_R, rgain_32);
    LOG_INF("[imx258_awb_gain] ABS_GAIN_B:%d, bgain_32:%d\n", pSetSensorAWB->ABS_GAIN_B, bgain_32);
    LOG_INF("[imx258_awb_gain] ABS_GAIN_GB:%d, gbgain_32:%d\n", pSetSensorAWB->ABS_GAIN_GB, gbgain_32);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			read_imx258_DCC((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
            break;
            ...
            switch (*feature_data_32) {
				...
				case MSDK_SCENARIO_ID_CUSTOM1:
 					if((imx258_type == IMX258_HDR_TYPE)||(imx258_type == IMX258_NOPDAF_TYPE))
						...
						memcpy((void *)wininfo,(void *)&imgsensor_winsize_info_Binning[5],sizeof(SENSOR_WINSIZE_INFO_STRUCT));
					break;
				case MSDK_SCENARIO_ID_CUSTOM2:
 					if((imx258_type == IMX258_HDR_TYPE)||(imx258_type == IMX258_NOPDAF_TYPE))
						memcpy((void *)wininfo,(void *)&imgsensor_winsize_info[6],sizeof(SENSOR_WINSIZE_INFO_STRUCT));
					else
						memcpy((void *)wininfo,(void *)&imgsensor_winsize_info_Binning[6],sizeof(SENSOR_WINSIZE_INFO_STRUCT));
					break;
				case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
				default:
					if((imx258_type == IMX258_HDR_TYPE)||(imx258_type == IMX258_NOPDAF_TYPE))
...
UINT32 IMX258_MIPI_RAW_MAIN_537_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
