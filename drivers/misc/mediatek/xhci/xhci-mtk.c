...
#ifdef CONFIG_NUBIA_CHARGING_COMMON
#include <mach/general_charger.h>
#endif
...
//#define NUBIA_OTG_DEBUG
#ifdef NUBIA_OTG_DEBUG
#define PR_DEBUG_NUBIA(fmt, args...) printk(KERN_ALERT "nubia_OTG: FILE:%s LINE:%d FUNC:%s " fmt, __FILE__ , __LINE__ , __func__ , ##args)
#else
#define PR_DEBUG_NUBIA(fmt, args...)
#endif
...
static int nubia_set_pmic_otg_mode( bool val)
{
...
static void mtk_enable_otg_mode(void)
{
	...
	bq25890_set_boost_ilim(0x03);	/* 1.3A */
#elif defined(CONFIG_TI_BQ25890_CHARGER_SUPPORT)
#ifdef CONFIG_MTK_LEGACY
        mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN, GPIO_OTG_DRVVBUS_PIN_M_GPIO);
        mt_set_gpio_out(GPIO_OTG_DRVVBUS_PIN, GPIO_OUT_ONE);
#else
        ...
        mt_set_gpio_out(0x80000043, GPIO_OUT_ONE);
#endif
        PR_DEBUG_NUBIA("NUBIA ENABLE PMIC OTG MODE\n");
        nubia_set_pmic_otg_mode( 1 );
...
#endif
#elif defined(CONFIG_TI_BQ25890_CHARGER_SUPPORT)
        PR_DEBUG_NUBIA("NUBIA DISABLE PMIC OTG MODE\n");
        nubia_set_pmic_otg_mode( 0 );
#ifdef CONFIG_MTK_LEGACY
        mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN, GPIO_OTG_DRVVBUS_PIN_M_GPIO);
        mt_set_gpio_out(GPIO_OTG_DRVVBUS_PIN, GPIO_OUT_ZERO);
#else
