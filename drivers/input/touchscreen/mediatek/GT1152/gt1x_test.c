...
int ini_read_int(char *file, const char *key)
{
	...
	value = gt1x_atoi((const char *)tmp);
...
* Return: char number
***********************************************************************/
static ssize_t gtp_fwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;
    u8 id_buffer[2] = { 0 };
    u8 cfg_buffer[1] = { 0 };
    ret =  gt1x_i2c_read(0x814A, id_buffer, sizeof(id_buffer));
    if (ret < 0) {
        return snprintf(buf, PAGE_SIZE, "Get ID's info failed!\n");
        }

    ret =  gt1x_i2c_read(0x8050, cfg_buffer, sizeof(cfg_buffer));
    if (ret < 0) {
        return snprintf(buf, PAGE_SIZE, "Get cfg's info failed!\n");
        }
	
	struct gt1x_version_info ver_info;
	ret = gt1x_read_version(&ver_info);
    if (ret < 0) {
		return snprintf(buf, PAGE_SIZE,"Get IC's version info failed!\n");
	}
	return snprintf(buf, PAGE_SIZE, "FW:GT%s_%06X\nCFG:%d\nID:0x%02X%02X\n",
	       ver_info.product_id, ver_info.patch_id,cfg_buffer[0],id_buffer[0],id_buffer[1]);
}

static DEVICE_ATTR(openshort, S_IRUGO | S_IWUSR, gtp_sysfs_openshort_show, gtp_sysfs_openshort_store);
static DEVICE_ATTR(gtpfwver, S_IRUGO|S_IWUSR, gtp_fwver_show, NULL);

/* add your attr in here*/
static struct attribute *gtp_attributes[] = {
	&dev_attr_gtpfwver.attr,
	&dev_attr_openshort.attr,
	NULL
};
...
s32 gtp_test_sysfs_init(void)
{
	...
	ret = sysfs_create_group(gt1x_debug_kobj,&gtp_attribute_group);
...
void gtp_test_sysfs_deinit(void)
{
	...
	sysfs_remove_group(gt1x_debug_kobj,&gtp_attribute_group);
