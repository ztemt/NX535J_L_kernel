...
static bool _read_3P3_eeprom1(kal_uint16 addr,  kal_uint32 size ){
	int i = 0;
	int offset = addr;
	for(i = 0; i < size; i++) {
		if(!selective_read_eeprom(offset, &s5k3P3_eeprom_data[i])){
			return false;
		}
		LOG_INF("read_eeprom 0x%0x s5k3P3_eeprom_data[%d] = %d\n",offset, i,s5k3P3_eeprom_data[i]);
		offset++;
	}
	get_done = true;
	last_size = size;
...
static bool _read_3P3_eeprom2(kal_uint16 addr, kal_uint32 size ){
	int i = 0;
	int offset = addr;
	for(i = 0; i < size; i++) {
		if(!selective_read_eeprom(offset, &s5k3P3_eeprom_data[496+i])){
			return false;
		}
		LOG_INF("read_eeprom 0x%0x s5k3P3_eeprom_data[%d] = %d\n",offset, 496+i,s5k3P3_eeprom_data[496+i]);
		offset++;
	}
	get_done = true;
	last_size = size;
...
bool read_3P3_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size){
	...
	//if(!get_done || last_size != size || last_offset != addr) {
       if(!get_done ) {	
	    addr = 0x791;
	    size = 496;	   	
	    if(!_read_3P3_eeprom1(addr, size)){
	        get_done = 0;
	        last_size = 0;
	        last_offset = 0;
	        return false;
	    ...
	    addr = 0x983;
	    size = 908;		
	    if(!_read_3P3_eeprom2(addr, size)){
	        get_done = 0;
	        last_size = 0;
	        last_offset = 0;
	        return false;
