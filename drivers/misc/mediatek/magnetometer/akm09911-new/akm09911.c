...
extern int bmi160_o_enable(int en); extern int bmi160_o_set_delay(u64 ns); 
 ...
 **************************/
	...
	#define MAG_TAG
	#undef MAG_FUN
	#define MAG_FUN(f)			do {} while (0)
	#undef MAG_ERR
	#define MAG_ERR(fmt, args...)	do {} while (0)
	#undef MAG_LOG
	#define MAG_LOG(fmt, args...)	do {} while (0)
	#undef MAG_VER
	#define MAG_VER(fmt, args...)   do{}while(0)
    #endif
...
{ 
	...
	{ 
		AKECS_GetData(sensordata, SENSOR_DATA_SIZE); 
		AKECS_SetMode_SngMeasure(); //msleep(10); 
	} else { 
		mutex_lock(&sense_data_mutex); 
		memcpy(sensordata, sense_data, sizeof(sensordata)); 
		mutex_unlock(&sense_data_mutex); 
	...
	/* printk("mag raw data[%d, %d, %d][%d, %d, %d]\n", databuf[AKM_AXIS_X], databuf[AKM_AXIS_Y], databuf[AKM_AXIS_Z], mag[AKM_AXIS_X], mag[AKM_AXIS_Y], mag[AKM_AXIS_Z]); */ 
	sprintf(strbuf, "%d %d %d\n", mag[0], mag[1], mag[2]); 
...
static int akm09911_m_get_data(int* x ,int* y,int* z, int* status)
{
	...
	*status = sensor_data[8];
	#if AKM09911_DEBUG
	printk("%s x: %d, y: %d, z: %d,", __func__, sensor_data[5], sensor_data[6], sensor_data[7]);
	#endif
