...
static ssize_t synaptics_rmi4_f54_ic_detect_show(struct device *dev,
...
//step 2: type 5, echo 5
	retval = read_report_select_type(F54_TX_TO_TX_SHORT);
	if (retval < 0){
...
//step 3: type 7, echo 7
	retval = read_report_select_type(F54_RX_TO_RX1);
	if (retval < 0){
...
//step 3: type 17, echo 17
	retval = read_report_select_type(F54_RX_TO_RX2);
	if (retval < 0){
