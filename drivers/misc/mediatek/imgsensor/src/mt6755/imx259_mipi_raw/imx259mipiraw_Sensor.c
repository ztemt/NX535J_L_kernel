...
#define LOG_1 LOG_INF("IMX259,MIPI 4LANE\n")
...
static BYTE imx259_SPC_data[252]={0};

extern void read_imx259_SPC( BYTE* data );
extern void read_imx259_DCC( kal_uint16 addr,BYTE* data, kal_uint32 size);

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX259_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define IMX259_NOPDAF_TYPE	(0x30)
kal_uint16 imx259_type = 0;/*0x00=HDR type, 0x10=binning type*/
...
static void imx259_apply_SPC(void)
{
	int i;

	if(read_spc_flag == FALSE)
	{
		read_imx259_SPC(imx259_SPC_data);
		read_spc_flag = TRUE;
		return;
	}
	...
	for(i=0; i<63; i++)
	{
		write_cmos_sensor(0xD04C+i, imx259_SPC_data[i]);
		LOG_INF("SPC_Data[%d] = %d\n", i, imx259_SPC_data[i]);
		//LOG_INF("read_SPC_Data[%d] = %d\n", i, read_cmos_sensor(0xD04C+i));
	}
	for(i=0; i<63; i++)
	{
		write_cmos_sensor(0xD08C+i, imx259_SPC_data[i+63]);
		LOG_INF("SPC_Data[%d] = %d\n", i+63, imx259_SPC_data[i+63]);
   ...
   while(retry--)
	{
		 if(read_cmos_sensor(0x0A01) == 0x01)
		 {
             ...
             imx259_type = IMX259_NOPDAF_TYPE;//ZTEMT:wangkai disable front cam PDAF
			 LOG_INF("imx259 type = 0x%x(0x00=HDR,0x10=binning,0x30=No PDAF)", imx259_type);
...
{
	if((imx259_type == IMX259_HDR_TYPE)||(imx259_type == IMX259_NOPDAF_TYPE))
	{
...
static void sensor_init(void)
{
	...
	if(imx259_type != IMX259_NOPDAF_TYPE)
	{
		imx259_apply_SPC();
...
static kal_uint32 imx259_awb_gain(SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
    
    UINT32 rgain_32, grgain_32, gbgain_32, bgain_32;

LOG_INF("imx259_awb_gain\n");
    ...
    LOG_INF("[imx259_awb_gain] ABS_GAIN_GR:%d, grgain_32:%d\n", pSetSensorAWB->ABS_GAIN_GR, grgain_32);
    LOG_INF("[imx259_awb_gain] ABS_GAIN_R:%d, rgain_32:%d\n", pSetSensorAWB->ABS_GAIN_R, rgain_32);
    LOG_INF("[imx259_awb_gain] ABS_GAIN_B:%d, bgain_32:%d\n", pSetSensorAWB->ABS_GAIN_B, bgain_32);
    LOG_INF("[imx259_awb_gain] ABS_GAIN_GB:%d, gbgain_32:%d\n", pSetSensorAWB->ABS_GAIN_GB, gbgain_32);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			//read_imx259_DCC((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
