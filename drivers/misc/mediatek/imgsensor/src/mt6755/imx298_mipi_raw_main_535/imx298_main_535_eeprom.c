...
#include "imx298_main_535_eeprom.h"
...
static bool _read_imx298_eeprom(kal_uint16 addr, BYTE* data, int size ){
	...
	for(i = 0; i < size; i++) {
		...
		CAM_CALDB("_read_imx298_eeprom 0x%0x 0x%x \n",offset, data[i]);
		offset++;
	}
//	get_done = true;
//	last_size = size;
...
void read_imx298_SPC(BYTE* data){
	...
	CAM_CALDB("read imx298 SPC, size = %d, get_done = %d, last_size = %d, last_offset = %d\n", size, get_done, last_size, last_offset);
	
	if(!get_done || last_size != size || last_offset != addr) {
	//	if(!_read_imx298_eeprom(addr, imx298_SPC_data, size)){
              if(!selective_read_region(addr, imx298_SPC_data, imx298_EEPROM_READ_ID,size)){			
		    get_done = 0;
	           last_size = 0;
	           last_offset = 0;
	           return false;
 ...
 //      CAM_CALDB("imx298_SPC_data[0] = 0x%x \n",imx298_SPC_data[0]);
//	for(i=1;i<253;i++)
//	{
//	    CAM_CALDB("imx298_SPC_data[%d] = 0x%x \n",i,imx298_SPC_data[i]);
//           sum += imx298_SPC_data[i];
//
//	}

//	CAM_CALDB("imx298_SPC_data[253] = 0x%x  sum = 0x%x \n",imx298_SPC_data[253],sum);
...
void read_imx298_DCC( kal_uint16 addr,BYTE* data, kal_uint32 size){
	...
	CAM_CALDB("read_imx298_DCC, size = %d, get_done = %d, last_size = %d, last_offset = %d \n", size, get_done, last_size, last_offset);
	
	//if(!get_done || last_size != size || last_offset != addr) {
	 if(!get_done) {	
	//	if(!_read_imx298_eeprom(addr, imx298_DCC_data, size)){
              if(!selective_read_region(addr, imx298_DCC_data, imx298_EEPROM_READ_ID,size)){			
			get_done = 0;
			last_size = 0;
			last_offset = 0;
			return false;
      ...
      //  CAM_CALDB("imx298_DCC_data[0] = 0x%x \n",imx298_DCC_data[0]);
//	for(i=0;i<96;i++)
//	{
//	    CAM_CALDB("imx298_DCC_data[%d] = 0x%x \n",i,imx298_DCC_data[i]);
        //   sum += imx298_DCC_data[i];

//	}

	//CAM_CALDB("imx298_DCC_data[97] = 0x%x  sum = 0x%x \n",imx298_DCC_data[97],sum);
//	CAM_CALDB("imx298_DCC_data[97] = 0x%x  sum = 0x%x \n",imx298_DCC_data[97],sum);
...
static bool byteread_cmos_sensor(unsigned char SLAVEID, unsigned short addr, unsigned char *data)
{
	...
	if (iReadRegI2C(puSendCmd , 2, data, 1, SLAVEID) < 0) {
		CAM_CALDB("[CAM_CAL] fail imx298_byteread_cmos_sensor addr =0x%x, data = 0x%x", addr, *data);
		return false;
	}
	/*CAM_CALDB("selective_read_byte addr =0x%x data = 0x%x,page %d, offset 0x%x", addr,
	*data,page,offset);*/
	//CAM_CALDB("[CAM_CAL] imx298_cmos_sensor addr =0x%x, data = 0x%x", addr, *data);
	return true;
...
static long imx298_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long ret;
	COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
	stCAM_CAL_INFO_STRUCT __user *data;
	int err;
	CAM_CALDB("[CAMERA SENSOR] imx298_Ioctl_Compat,%p %p %x ioc size %d\n", filp->f_op ,
...
CAM_CALDB("[CAM_CAL] ioctl\n");
	...
	case CAM_CALIOC_G_READ:
		...
		/**pu1Params = 0;*/
		readTryagain = 3;
		//i4RetValue = selective_read_region(ptempbuf->u4Offset, pu1Params, IMX298_DEVICE_ID, ptempbuf->u4Length);
#if 0
...
#else
		i4RetValue = selective_read_region(ptempbuf->u4Offset, pWorkingBuff, imx298_EEPROM_READ_ID, ptempbuf->u4Length);
#endif
#ifdef CAM_CALGETDLT_DEBUG
		do_gettimeofday(&ktv2);
		if (ktv2.tv_sec > ktv1.tv_sec)
...
static const struct file_operations g_stCAM_CAL_fops = {
	...
	/*.ioctl = CAM_CAL_Ioctl*/
#ifdef CONFIG_COMPAT
	.compat_ioctl = imx298_Ioctl_Compat,
#endif
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclient->addr = imx298_EEPROM_READ_ID >> 1;
