...
//#define ztemt_audio_headset_eu_us_support
...
{
	/*enable ACCDET unit*/
	ACCDET_ERROR("accdet: enable_accdet\n");
	/*enable clock*/
...
static void accdet_eint_work_callback(struct work_struct *work)
{
	...
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
		ACCDET_ERROR("[Accdet]DCC EINT func :plug-in, cur_eint_state = %d\n", cur_eint_state);
		mutex_lock(&accdet_eint_irq_sync_mutex);
...
/*EINT_PIN_PLUG_OUT*/
/*Disable ACCDET*/
		ACCDET_ERROR("[Accdet]DCC EINT func :plug-out, cur_eint_state = %d\n", cur_eint_state);
		mutex_lock(&accdet_eint_irq_sync_mutex);
...
#ifdef ACCDET_EINT_IRQ
	ACCDET_ERROR("[Accdet accdet_irq_handler]clear_accdet_eint_interrupt: ACCDET_IRQ_STS = 0x%x\n",
...
static inline void clear_accdet_interrupt(void)
{
	...
	ACCDET_ERROR("[Accdet]clear_accdet_interrupt: ACCDET_IRQ_STS = 0x%x\n", pmic_pwrap_read(ACCDET_IRQ_STS));
}
...
#define PIN_ADC_CHANNEL 5
	...
	ACCDET_ERROR("[Accdet]accdet interrupt happen:[%s]current AB = %d\n",
		...
		if (current_status == 0) {
			...
			if (1 == eint_accdet_sync_flag) {
              ...
              if(state_eu_us_switch==0){
							...
							pr_err("nubia [Accdet] switch to us %d mode \n",state_eu_us_switch);
			  }
#endif
	...
	if (!IRQ_CLR_FLAG) {
		...
		mutex_unlock(&accdet_eint_irq_sync_mutex);
		IRQ_CLR_FLAG = TRUE;
		ACCDET_ERROR("[Accdet]check_cable_type:Clear interrupt:Done[0x%x]!\n", pmic_pwrap_read(ACCDET_IRQ_STS));

	} else {
		IRQ_CLR_FLAG = FALSE;
	}

	ACCDET_ERROR("[Accdet]cable type:[%s], status switch:[%s]->[%s]\n",
...
static void accdet_work_callback(struct work_struct *work)
{
	...
	mutex_unlock(&accdet_eint_irq_sync_mutex);
	ACCDET_ERROR(" [accdet] set state in cable_type  status\n");
...
void accdet_int_handler(void)
{
	...
	ACCDET_ERROR("[accdet_int_handler]....\n");
